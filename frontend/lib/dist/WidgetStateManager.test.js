...
import { ArrowTable as ArrowTableProto, Button as ButtonProto, FileUploaderState as FileUploaderStateProto, UploadedFileInfo as UploadedFileInfoProto } from "./proto";
import { createFormsData, WidgetStateDict, WidgetStateManager } from "./WidgetStateManager";
...
const MOCK_FILE_UPLOADER_STATE = new FileUploaderStateProto({
  maxFileId: 42,
  uploadedFileInfo: [new UploadedFileInfoProto({
    id: 4,
    name: "bob",
    size: 5
...
describe("Widget State Manager", () => {
  ...
  const assertCallbacks = _ref2 => {
    ...
    if (insideForm) {
      ...
      expect(sendBackMsg).toHaveBeenCalledWith(expect.anything(), undefined // fragmentId
      );
    }
  };
  it.each([false, true])("sets string value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
    widgetMgr.setStringValue(widget, "mockStringValue", {
      fromUi: true
    }, undefined);
  ...
  it.each([false, true])("sets boolean value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
    widgetMgr.setBoolValue(widget, true, {
      fromUi: true
    }, undefined);
  ...
  it.each([false, true])("sets int value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
    widgetMgr.setIntValue(widget, 100, {
      fromUi: true
    }, undefined);
  ...
  it.each([false, true])("sets float value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
    widgetMgr.setDoubleValue(widget, 3.14, {
      fromUi: true
    }, undefined);
  ...
  it("sets trigger value correctly", () => {
    ...
    widgetMgr.setTriggerValue(widget, {
      fromUi: true
    }, undefined);
    // @ts-expect-error
  ...
  it("sets string trigger value correctly", () => {
    ...
    widgetMgr.setStringTriggerValue(widget, "sample string", {
      fromUi: true
    }, undefined);
    // @ts-expect-error
    expect(widgetMgr.getWidgetState(widget)).toBe(undefined);
  ...
  it.each([false, true])("sets string array value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
    ...
    expect(widgetMgr.getStringArrayValue(widget)).toEqual(["foo", "bar", "baz"]);
    assertCallbacks({
      insideForm
    });
  });
  it.each([false, true])("sets int array value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
    widgetMgr.setIntArrayValue(widget, [4, 5, 6], {
      fromUi: true
    }, undefined);
  ...
  it.each([false, true])("sets float array value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
  ...
  it.each([false, true])("sets ArrowTable value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
  ...
  it.each([false, true])("sets JSON value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
  ...
  it.each([false, true])("sets bytes value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
    widgetMgr.setBytesValue(widget, MOCK_BYTES, {
      fromUi: true
    }, undefined);
  ...
  it.each([false, true])("sets FileUploaderState value correctly (insideForm=%p)", insideForm => {
    const widget = getWidget({
      insideForm
    });
    ...
    expect(widgetMgr.getFileUploaderStateValue(widget)).toEqual(MOCK_FILE_UPLOADER_STATE);
    assertCallbacks({
      insideForm
    });
  });
  ...
  describe("can set fragmentId in setter methods", () => {
    it.each([{
      ...
      // @ts-expect-error
      widgetMgr[setterMethod](MOCK_WIDGET, value, {
        fromUi: true
      }, "myFragmentId");
      expect(sendBackMsg).toHaveBeenCalledWith(expect.anything(), "myFragmentId");
    });
    ...
    it("can set fragmentId in setTriggerValue", () => {
      widgetMgr.setTriggerValue(MOCK_WIDGET, {
        fromUi: true
      }, "myFragmentId");
      expect(sendBackMsg).toHaveBeenCalledWith(expect.anything(), "myFragmentId");
    });
  ...
  describe("Primitive types as JSON values", () => {
    it("sets string value as JSON correctly", () => {
      widgetMgr.setJsonValue(MOCK_WIDGET, "mockStringValue", {
        fromUi: true
      }, undefined);
      expect(widgetMgr.getJsonValue(MOCK_WIDGET)).toBe(JSON.stringify("mockStringValue"));
    });
    it("sets int value as JSON correctly", () => {
      widgetMgr.setJsonValue(MOCK_WIDGET, 45, {
        fromUi: true
      }, undefined);
      expect(widgetMgr.getJsonValue(MOCK_WIDGET)).toBe(JSON.stringify(45));
    });
    it("sets float value as JSON correctly", () => {
      widgetMgr.setJsonValue(MOCK_WIDGET, 3.14, {
        fromUi: true
      }, undefined);
    ...
    it("sets string array value as JSON correctly", () => {
      ...
      expect(widgetMgr.getJsonValue(MOCK_WIDGET)).toBe(JSON.stringify(["foo", "bar", "baz"]));
    });
    it("sets int array value as JSON correctly", () => {
      widgetMgr.setJsonValue(MOCK_WIDGET, [5, 6, 7], {
        fromUi: true
      }, undefined);
      expect(widgetMgr.getJsonValue(MOCK_WIDGET)).toBe(JSON.stringify([5, 6, 7]));
    });
    it("sets float array value as JSON correctly", () => {
      widgetMgr.setJsonValue(MOCK_WIDGET, [1.1, 2.2, 3.3], {
        fromUi: true
      }, undefined);
      expect(widgetMgr.getJsonValue(MOCK_WIDGET)).toBe(JSON.stringify([1.1, 2.2, 3.3]));
    });
  });
  ...
  describe("formsData", () => {
    it("updates submitButtons Array", () => {
      ...
      const secondButtonMock = new ButtonProto({
        id: "newId"
      });
      expect(formsData.submitButtons.get("form")).not.toBeDefined();
      widgetMgr.addSubmitButton("form", newButtonMock);
      expect((_formsData$submitButt = formsData.submitButtons.get("form")) === null || _formsData$submitButt === void 0 ? void 0 : _formsData$submitButt.length).toEqual(1);
      widgetMgr.addSubmitButton("form", secondButtonMock);
      expect((_formsData$submitButt2 = formsData.submitButtons.get("form")) === null || _formsData$submitButt2 === void 0 ? void 0 : _formsData$submitButt2.length).toEqual(2);
      widgetMgr.removeSubmitButton("form", newButtonMock);
      expect((_formsData$submitButt3 = formsData.submitButtons.get("form")) === null || _formsData$submitButt3 === void 0 ? void 0 : _formsData$submitButt3.length).toEqual(1);
      widgetMgr.removeSubmitButton("form", secondButtonMock);
      expect((_formsData$submitButt4 = formsData.submitButtons.get("form")) === null || _formsData$submitButt4 === void 0 ? void 0 : _formsData$submitButt4.length).toEqual(0);
    });
  ...
  describe("submitForm", () => {
    it("calls sendBackMsg with expected data", () => {
      ...
      widgetMgr.addSubmitButton(formId, new ButtonProto({
        id: "submitButton"
      }));
    ...
    it("throws on invalid formId", () => {
      expect(() => widgetMgr.submitForm(MOCK_WIDGET.formId, undefined)).toThrow("invalid formID ".concat(MOCK_WIDGET.formId));
    });
  ...
  describe("Forms don't interfere with each other", () => {
    ...
    it("checks that there are two pending forms", () => {
      expect(formsData.formsWithPendingChanges).toEqual(new Set([FORM_1.formId, FORM_2.formId]));
    });
    it("calls sendBackMsg with the first form data", () => {
      widgetMgr.addSubmitButton(FORM_1.formId, new ButtonProto({
        id: "submitButton"
      }));
    ...
    it("checks that only the second form is pending after the first is submitted", () => {
      widgetMgr.submitForm(FORM_1.formId, undefined);
      expect(formsData.formsWithPendingChanges).toEqual(new Set([FORM_2.formId]));
    });
    it("calls sendBackMsg with data from both forms", () => {
      // Submit the first form and then the second form.
      widgetMgr.submitForm(FORM_1.formId, undefined);
      widgetMgr.submitForm(FORM_2.formId, undefined, new ButtonProto({
        id: "submitButton2"
      }));
    ...
    it("supports two submit buttons and can submitForm on the second one", () => {
      ...
      widgetMgr.addSubmitButton(FORM_2.formId, new ButtonProto({
        id: "submitButton2"
      }));
  ...
  describe("manages element state values", () => {
    it("sets extra widget information properly", () => {
      var _widgetMgr$elementSta;
      widgetMgr.setElementState("id", "color", "red");
      // @ts-expect-error
      expect((_widgetMgr$elementSta = widgetMgr.elementStates.get("id")) === null || _widgetMgr$elementSta === void 0 ? void 0 : _widgetMgr$elementSta.get("color")).toEqual("red");
    });
  ...
  it("cleans up widget & element states on removeInactive", () => {
    ...
    expect(widgetMgr.getStringValue({
      id: widgetId3
    })).toEqual("widgetState3");
    expect(widgetMgr.getStringValue({
      id: widgetId4
    })).toEqual("widgetState4");
    expect(widgetMgr.getElementState(elementId1, "key1")).toBeUndefined();
    expect(widgetMgr.getElementState(elementId2, "key2")).toEqual("elementState2");
  });
});
describe("WidgetStateDict", () => {
  ...
  it("creates a new state with the given widget id", () => {
    ...
    expect(widgetStateDict.getState(widgetId)).toEqual({
      id: widgetId
    });
  });
  ...
  it("cleans states of widgets that are not contained in `activeIds`", () => {
    ...
    expect(widgetStateDict.getState(widgetId3)).toEqual({
      id: widgetId3
    });
    expect(widgetStateDict.getState(widgetId4)).toEqual({
      id: widgetId4
    });
  });
  ...
  it("copies the contents of another WidgetStateDict into the given one, overwriting any values with duplicate keys", () => {
    ...
    expect(widgetStateDict.getState(widgetId1)).toEqual({
      id: widgetId1
    });
    expect(widgetStateDict.getState(widgetId2)).toEqual({
      id: widgetId2
    });
    expect(widgetStateDict.getState(widgetId3)).toEqual({
      id: widgetId3
...
//# sourceMappingURL=WidgetStateManager.test.js.map