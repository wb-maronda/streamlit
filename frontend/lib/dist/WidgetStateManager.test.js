...
describe("Widget State Manager", () => {
  ...
  const assertCallbacks = _ref2 => {
    ...
    if (insideForm) {
      ...
      expect(sendBackMsg).toHaveBeenCalledWith(expect.anything(), undefined,
      // fragmentId
      undefined, undefined);
    }
  };
  ...
  describe("can set fragmentId in setter methods", () => {
    it.each([{
      ...
      expect(sendBackMsg).toHaveBeenCalledWith(expect.anything(), "myFragmentId", undefined, undefined);
    });
  ...
  describe("submitForm", () => {
    ...
    it("calls sendBackMsg with fragmentId", () => {
      ...
      }, "myFragmentId", undefined, undefined);

      // We have no more pending form.
      expect(formsData.formsWithPendingChanges).toEqual(new Set());
    });
    it("throws on invalid formId", () => {
      expect(() => widgetMgr.submitForm(MOCK_WIDGET.formId, undefined)).toThrow(`invalid formID ${MOCK_WIDGET.formId}`);
    });
