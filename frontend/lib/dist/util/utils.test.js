...
import { EMBED_QUERY_PARAM_KEY, EMBED_QUERY_PARAM_VALUES, getCookie, getEmbedUrlParams, getLoadingScreenType, isColoredLineDisplayed, isDarkThemeInQueryParams, isEmbed, isLightThemeInQueryParams, isPaddingDisplayed, isScrollingHidden, isToolbarDisplayed, keysToSnakeCase, LoadingScreenType, preserveEmbedQueryParams, setCookie } from "./utils";
describe("getCookie", () => {
  afterEach(() => {
...
describe("embedParamValues", () => {
  const embedParamValuesShouldHave = ["show_colored_line", "show_toolbar", "show_padding", "disable_scrolling", "light_theme", "dark_theme", "hide_loading_screen", "show_loading_screen_v1", "show_loading_screen_v2", "true"];
  it("embedParamValues have correct values", () => {
    expect(EMBED_QUERY_PARAM_VALUES.length).toBe(embedParamValuesShouldHave.length);
...
describe("isEmbed", () => {
  ...
  it("embed Options should return false even if ?embed=false", () => {
    windowSpy.mockImplementation(() => ({
      location: {
        search: "?embed=false&embed_options=show_colored_line,show_toolbar,show_padding,disable_scrolling"
      }
    }));
    expect(isColoredLineDisplayed()).toBe(false);
    expect(isToolbarDisplayed()).toBe(false);
  ...
  it("embed Options should return false even if ?embed is not set", () => {
    windowSpy.mockImplementation(() => ({
      location: {
        search: "?embed_options=show_colored_line,show_toolbar,show_padding,disable_scrolling"
      }
    }));
    expect(isColoredLineDisplayed()).toBe(false);
    expect(isToolbarDisplayed()).toBe(false);
...
describe("getLoadingScreenType", () => {
  ...
  it("should give precendence to 'hide'", () => {
    windowSpy.mockImplementation(() => ({
      location: {
        search: "?embed_options=hide_loading_screen&show_loading_screen_v1&show_loading_screen_v2"
      }
    }));
  ...
  describe("preserveEmbedQueryParams", () => {
    ...
    it("should preserve embed query string with embed options and remove foo=bar", () => {
      ...
      window.location = {
        assign: jest.fn(),
        search: "embed=true&embed_options=option1&embed_options=option2&foo=bar"
      };
      expect(preserveEmbedQueryParams()).toBe("embed=true&embed_options=option1&embed_options=option2");
    });
  });
});
describe("keysToSnakeCase", () => {
  it("should replace . with _", () => {
    expect(keysToSnakeCase({
      "marker.size": "bob"
    })).toEqual({
      marker_size: "bob"
    });
