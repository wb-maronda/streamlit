...
export function setCookie(name, value, expiration) {
  ...
  const expirationStr = expirationDate ? `expires=${expirationDate.toUTCString()};` : "";
...
export function getElementId(element) {
  const elementId = get(element, [requireNonNull(element.type), "id"]);
...
export function extractPageNameFromPathName(pathname, basePath) {
  ...
  return decodeURIComponent(document.location.pathname.replace(`/${basePath}`, "").replace(new RegExp("^/?"), "").replace(new RegExp("/$"), ""));
}
