 ...
 */
export function getWindowBaseUriParts() {
  ...
  const basePath = window.location.pathname.replace(FINAL_SLASH_RE, "").replace(INITIAL_SLASH_RE, "");
  return {
    host,
    port,
    basePath
...
export function getPossibleBaseUris() {
 ...
 */
export function buildWsUri(_ref, path) {
  let {
    host,
    port,
    basePath
  ...
  return "".concat(protocol, "://").concat(host, ":").concat(port, "/").concat(fullPath);
}

/**
 * Create an HTTP URI for the given path.
 */
export function buildHttpUri(_ref2, path) {
  let {
    host,
    port,
    basePath
...
function makePath(basePath, subPath) {
  ...
  return "".concat(basePath, "/").concat(subPath);
}
 ...
 * ignored.
 */
export function isValidOrigin(allowedOrigin, testOrigin) {
  let allowedUrl;
  let testUrl;
  try {
  ...
  if (allowedUrl.protocol !== testUrl.protocol || allowedUrl.port !== testUrl.port && testUrl.hostname !== "localhost") {
    return false;
  }
  const {
    hostname: pattern
