...
import { buildHttpUri, buildWsUri, getPossibleBaseUris, getWindowBaseUriParts, isValidOrigin } from "./UriUtil";
const location = {};
...
describe("isValidOrigin", () => {
  it("returns false if allowedOrigin is invalid", () => {
    ...
    expect(isValidOrigin("devel.streamlit.io", "http://devel.streamlit.io")).toBeFalsy();
  });
  it("returns false if testOrigin is invalid", () => {
    // testOrigin doesn't have a protocol
    expect(isValidOrigin("http://devel.streamlit.io", "devel.streamlit.io")).toBeFalsy();
  });
  it("returns true if testUrl's hostname is localhost w/ various ports", () => {
    expect(isValidOrigin("http:localhost",
    // Example of localhost url used for manual testing
    "http://localhost:8000")).toBeTruthy();
    expect(isValidOrigin("http://localhost",
    // Example of localhost url used by e2e test
    "http://localhost:35475")).toBeTruthy();
  });
  it("returns false if protocols don't match", () => {
    expect(isValidOrigin("https://devel.streamlit.io", "http://devel.streamlit.io")).toBeFalsy();
  });
  it("returns false if ports don't match", () => {
    expect(isValidOrigin("https://devel.streamlit.io:8080", "https://devel.streamlit.io")).toBeFalsy();
  });
  it("returns true when the pattern and url are the same", () => {
    expect(isValidOrigin("http://devel.streamlit.io", "http://devel.streamlit.io")).toBeTruthy();
  });
  ...
  it("returns true if it matches the pattern", () => {
    ...
    expect(isValidOrigin("https://www.*.a.com", "https://www.asd.a.com")).toBeTruthy();
    expect(isValidOrigin("https://abc.*.*.a.com", "https://abc.def.xyz.a.com")).toBeTruthy();
  });
...
describe("getPossibleBaseUris", () => {
  ...
  testCases.forEach(_ref => {
    ...
    it("handles ".concat(description), () => {
      window.location.pathname = pathname;
      expect(getPossibleBaseUris().map(b => b.basePath)).toEqual(expectedBasePaths);
    });
  });
});
//# sourceMappingURL=UriUtil.test.js.map