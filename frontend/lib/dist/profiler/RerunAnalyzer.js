...
import { findNextEventIndex, findPrevEvent, findPrevEventIndex, getTimeDelta, isHandleMessageEvent } from "./Utils";
class RerunAnalyzer {
  /** All the events that occurred in this rerun. */

  /** The rerun request event that immediately preceded the rerun, if any. */

  constructor(allEvents, lastEventIndex) {
    this.rerunEvents = void 0;
    this.requestedRerun = void 0;
    this.getResults = () => {
      ...
      const sortedMessageIndexes = Array.from(messageIndexes).sort(compareNumbers);
      ...
      if (this.requestedRerun !== undefined) {
        ...
        results.requestToRerunStart = getTimeDelta(this.requestedRerun, firstRerunEvent);
      }
      return results;
    };
    this.getMessageAnalysis = messageIndex => {
      // Get each event associated with the given message.
      const handleMessageEvents = [];
      let curIndex = 0;
      while (curIndex < this.rerunEvents.length) {
        const nextEventIndex = findNextMessageEvent(this.rerunEvents, curIndex, messageIndex);
        if (nextEventIndex === undefined) {
          break;
        }
        handleMessageEvents.push(this.rerunEvents[nextEventIndex]);
        curIndex = nextEventIndex + 1;
      }
      if (handleMessageEvents.length === 0) {
        throw new Error("No messages for the given index: ".concat(messageIndex));
      }
    ...
    this.requestedRerun = findPrevEvent(allEvents, firstEventIndex - 1, evt => evt.name === "RequestedRerun");
  }
}
...
function findRunStartEventIndex(events, lastEventIndex) {
  const newSessionDecodedIndex = findPrevEventIndex(events, lastEventIndex - 1, event => event.name === "DecodedMessage" && event.messageType === "newSession");
  ...
  const {
    messageIndex
  } = events[newSessionDecodedIndex];
  return findPrevEventIndex(events, newSessionDecodedIndex, event => event.name === "BeginHandleMessage" && event.messageIndex === messageIndex);
}

/** Find the next HandleMessageEvent index for the given message. */
function findNextMessageEvent(events, startIndex, messageIndex) {
  return findNextEventIndex(events, startIndex, evt => isHandleMessageEvent(evt) && evt.messageIndex === messageIndex);
}

/** Return a human-readable performance analysis of a single rerun. */
export function getRerunAnalysis(allEvents, lastEventIndex) {
  return new RerunAnalyzer(allEvents, lastEventIndex !== null && lastEventIndex !== void 0 ? lastEventIndex : allEvents.length - 1).getResults();
}
//# sourceMappingURL=RerunAnalyzer.js.map