...
import { Block as BlockProto, Delta as DeltaProto, Element, ForwardMsgMetadata, Logo as LogoProto } from "./proto";
import { AppRoot, BlockNode, ElementNode } from "./AppNode";
...
describe("ElementNode.vegaLiteChartElement", () => {
  it("returns a vegaLiteChartElement (data)", () => {
    ...
    expect(element.data?.data.toArray().map(a => a?.toArray())).toEqual([["foo", "1"], ["bar", "2"]]);
  ...
  it("returns a vegaLiteChartElement (datasets)", () => {
    ...
    expect(element.datasets[0].data.data.toArray().map(a => a?.toArray())).toEqual([["foo", "1"], ["bar", "2"]]);
...
describe("ElementNode.arrowAddRows", () => {
  ...
  describe("arrowTable", () => {
    test("addRows can be called with an unnamed dataset", () => {
      ...
      expect(q.columns).toEqual([["c1", "c2"]]);
      expect(q.data.toArray().map(a => a?.toArray())).toEqual([["foo", "1"], ["bar", "2"], ["foo", "1"], ["bar", "2"]]);
      expect(q.types).toEqual({
        index: [{
          pandas_type: IndexTypeName.UnicodeIndex,
          numpy_type: "object",
  ...
  describe("arrowVegaLiteChart", () => {
    ...
    describe("addRows is called with a named dataset", () => {
      test("element has one dataset -> append new rows to that dataset", () => {
        ...
        expect(element.datasets[0].data.columns).toEqual([["c1", "c2"]]);
        expect(element.datasets[0].data.data.toArray().map(a => a?.toArray())).toEqual([["foo", "1"], ["bar", "2"], ["foo", "1"], ["bar", "2"]]);
        expect(element.datasets[0].data.types).toEqual({
          index: [{
            pandas_type: IndexTypeName.UnicodeIndex,
            numpy_type: "object",
      ...
      test("element doesn't have a matched dataset, but has data -> append new rows to data", () => {
        ...
        const newNode = node.arrowAddRows(MOCK_NAMED_DATASET, NO_SCRIPT_RUN_ID);
        const element = newNode.vegaLiteChartElement;
        expect(element.data?.index).toEqual([vectorFromArray(["i1", "i2", "i1", "i2"])]);
        expect(element.data?.columns).toEqual([["c1", "c2"]]);
        expect(element.data?.data.toArray().map(a => a?.toArray())).toEqual([["foo", "1"], ["bar", "2"], ["foo", "1"], ["bar", "2"]]);
        expect(element.data?.types).toEqual({
          index: [{
            pandas_type: IndexTypeName.UnicodeIndex,
            numpy_type: "object",
...
describe("AppRoot.empty", () => {
  ...
  it("passes logo to new Root if empty is called with logo", async () => {
    const logo = LogoProto.create({
      image: "https://global.discourse-cdn.com/business7/uploads/streamlit/original/2X/8/8cb5b6c0e1fe4e4ebfd30b769204c0d30c332fec.png"
    });
...
describe("AppRoot.applyDelta", () => {
  ...
  it("removes a block's children if the block type changes for the same delta path", () => {
    const newRoot = ROOT.applyDelta("script_run_id", makeProto(DeltaProto, {
      addBlock: {
        expandable: {
          expanded: true,
          label: "label",
    ...
    }), forwardMsgMetadata([0, 1, 1])).applyDelta("script_run_id", makeProto(DeltaProto, {
      newElement: {
        text: {
          body: "newElement!"
        }
    ...
    const newRoot2 = newRoot.applyDelta("new_script_run_id", makeProto(DeltaProto, {
      addBlock: {
        tabContainer: {}
      }
    }), forwardMsgMetadata([0, 1, 1]));
    const replacedBlock = newRoot2.main.getIn([1, 1]);
...
describe("AppRoot.clearStaleNodes", () => {
  ...
  it("handles currentFragmentId correctly", () => {
    ...
    expect(pruned.main.getIn([2]).children).toHaveLength(1);
    expect(pruned.main.getIn([2, 0]).deltaBlock.tab?.label).toContain("tab1");
  });
  it("clear childNodes of a block node in fragment run", () => {
    ...
    .applyDelta("new_session_id", makeProto(DeltaProto, {
      ...
      fragmentId: "my_fragment_id"
    }), forwardMsgMetadata([0, 0])).applyDelta("new_session_id", delta, forwardMsgMetadata([0, 0, 0]))
    ...
    .applyDelta("new_session_id", makeProto(DeltaProto, {
      ...
      fragmentId: "my_fragment_id"
    }), forwardMsgMetadata([0, 1])).applyDelta("new_session_id", delta, forwardMsgMetadata([0, 1, 0])).applyDelta("new_session_id", delta, forwardMsgMetadata([0, 1, 1]))
    ...
    expect(newRoot.main.getIn([1]).children).toHaveLength(4);
    const pruned = newRoot.clearStaleNodes("new_session_id", ["my_fragment_id"]);
    expect(pruned.main.getIn([0])).toBeInstanceOf(BlockNode);
    expect(pruned.main.getIn([0]).children).toHaveLength(1);
    expect(pruned.main.getIn([1])).toBeInstanceOf(BlockNode);
    // the stale nested fragment child should have been pruned
    expect(pruned.main.getIn([1]).children).toHaveLength(3);
  });
});
...
expect.extend({
  toBeTextNode(received, text) {
    ...
    if (type !== "text") {
      return {
        message: () => `expected ${received}.element.type to be 'text', but it was ${type}`,
        pass: false
      };
    }
    const textBody = elementNode.element.text?.body;
    return {
      message: () => `expected ${received}.element.text.body to be "${text}", but it was "${textBody}"`,
      pass: textBody === text
    };
  }
});
