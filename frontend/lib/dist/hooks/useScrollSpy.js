 ...
 * @returns void.
 */
export default function useScrollSpy(target, eventHandler) {
  const onEventRef = useRef(eventHandler);
  const debouncer = useMemo(() => debounce(event => {
    onEventRef.current(event);
  }, DEFAULT_DEBOUNCE_MS), [onEventRef]);
  ...
  useLayoutEffect(() => {
    ...
    target.addEventListener("scroll", handleEvent, {
      passive: true
    });
    handleEvent({
      target
