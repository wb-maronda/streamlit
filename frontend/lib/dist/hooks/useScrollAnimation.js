 ...
 * @returns {void}
 */
export default function useScrollAnimation(target, onEnd, isAnimating) {
  const animator = useRef(0);
  const animate = useCallback(function (from, index) {
    let start = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : Date.now();
