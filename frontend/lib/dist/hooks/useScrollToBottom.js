...
export function useScrollToBottom() {
  ...
  }, [ignoreScrollEventBeforeRef, offsetHeightRef, scrollHeightRef, isAnimatingRef, isStickyRef, setIsAnimating, setIsSticky]);
  useEffect(() => {
    if (scrollableRef.current) {
      ...
      const timeout = setImmediateInterval(() => {
        ...
        } else if (target && target.scrollHeight <= target.offsetHeight && !isStickyRef.current) {
  ...
  }, [scrollableRef, isSticky, isAnimating, isAnimatingRef, isStickyRef, setIsSticky, setIsAnimating]);
  useEffect(() => {
  ...
  useScrollAnimation(scrollableRef.current, handleScrollToBottomFinished, isAnimating);
