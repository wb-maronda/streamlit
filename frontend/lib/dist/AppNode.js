...
import { getLoadingScreenType, isNullOrUndefined, LoadingScreenType, makeAppSkeletonElement, makeElementWithErrorText, makeElementWithInfoText, notUndefined } from "./util/utils";
...
export class ElementNode {
  ...
  get quiverElement() {
    ...
    if (this.element.type !== "arrowTable" && this.element.type !== "arrowDataFrame") {
      throw new Error(`elementType '${this.element.type}' is not a valid Quiver element!`);
    }
  ...
  get vegaLiteChartElement() {
    ...
    if (this.element.type !== "arrowVegaLiteChart") {
      throw new Error(`elementType '${this.element.type}' is not a valid VegaLiteChartElement!`);
    }
      ...
      default:
        {
          ...
          throw new Error(`elementType '${this.element.type}' is not a valid arrowAddRows target!`);
        }
    }
    return newNode;
  }
...
export class BlockNode {
  ...
  constructor(activeScriptHash, children, deltaBlock, scriptRunId, fragmentId, deltaMsgReceivedAt) {
    this.children = void 0;
    this.deltaBlock = void 0;
    this.scriptRunId = void 0;
    this.fragmentId = void 0;
    this.deltaMsgReceivedAt = void 0;
    this.activeScriptHash = void 0;
    this.activeScriptHash = activeScriptHash;
  ...
  setIn(path, node, scriptRunId) {
    ...
    const childIndex = path[0];
    if (childIndex < 0 || childIndex > this.children.length) {
      throw new Error(`Bad 'setIn' index ${childIndex} (should be between [0, ${this.children.length}])`);
    }
    ...
    return new BlockNode(this.activeScriptHash, newChildren, this.deltaBlock, scriptRunId, this.fragmentId, this.deltaMsgReceivedAt);
  }
  filterMainScriptElements(mainScriptHash) {
    if (this.activeScriptHash !== mainScriptHash) {
      return undefined;
    ...
    return new BlockNode(this.activeScriptHash, newChildren, this.deltaBlock, this.scriptRunId, this.fragmentId, this.deltaMsgReceivedAt);
  }
  clearStaleNodes(currentScriptRunId, fragmentIdsThisRun, fragmentIdOfBlock) {
    if (!fragmentIdsThisRun || !fragmentIdsThisRun.length) {
      ...
      if (this.fragmentId && fragmentIdsThisRun.includes(this.fragmentId) && this.scriptRunId === currentScriptRunId) {
    ...
    const newChildren = this.children.map(child => {
      return child.clearStaleNodes(currentScriptRunId, fragmentIdsThisRun, fragmentIdOfBlock);
    }).filter(notUndefined);
    return new BlockNode(this.activeScriptHash, newChildren, this.deltaBlock, currentScriptRunId, this.fragmentId, this.deltaMsgReceivedAt);
  }
...
export class AppRoot {
  ...
  static empty() {
    ...
    let logo = arguments.length > 3 ? arguments[3] : undefined;
    const mainNodes = [];
    let waitElement;
    ...
    return new AppRoot(mainScriptHash, new BlockNode(mainScriptHash, [main, sidebar, event, bottom]), appLogo);
  }
  constructor(mainScriptHash, root) {
    ...
    if (this.root.children.length !== 4 || isNullOrUndefined(this.main) || isNullOrUndefined(this.sidebar) || isNullOrUndefined(this.event) || isNullOrUndefined(this.bottom)) {
      ...
      case "addBlock":
        {
          ...
          return this.addBlock(deltaPath, delta.addBlock, scriptRunId, activeScriptHash, delta.fragmentId, deltaMsgReceivedAt);
        }
      case "arrowAddRows":
        {
          try {
  ...
  filterMainScriptElements(mainScriptHash) {
    ...
    const appLogo = this.appLogo?.activeScriptHash === mainScriptHash ? this.appLogo : null;
  ...
  clearStaleNodes(currentScriptRunId, fragmentIdsThisRun) {
    ...
    const appLogo = this.appLogo?.scriptRunId === currentScriptRunId ? this.appLogo : null;
  ...
  addBlock(deltaPath, block, scriptRunId, activeScriptHash, fragmentId, deltaMsgReceivedAt) {
    ...
    let children = [];
    if (existingNode instanceof BlockNode && existingNode.deltaBlock.type === block.type) {
      children = existingNode.children;
    }
    const blockNode = new BlockNode(activeScriptHash, children, block, scriptRunId, fragmentId, deltaMsgReceivedAt);
