...
import { baseTheme, createAutoTheme, darkTheme, lightTheme } from "./index";
import { fonts } from "./primitives/typography";
import { AUTO_THEME_NAME, bgColorToBaseString, CUSTOM_THEME_NAME, computeSpacingStyle, createEmotionTheme, createTheme, fontEnumToString, fontToEnum, getDefaultTheme, getSystemTheme, getWrappedHeadersStyle, isColor, isPresetTheme, toThemeInput, getCachedTheme, removeCachedTheme, setCachedTheme, hasLightBackgroundColor } from "./utils";
...
const windowLocationSearch = search => ({
  location: {
    search
  }
});
const windowMatchMedia = theme => ({
  matchMedia: query => ({
    matches: query === "(prefers-color-scheme: ".concat(theme, ")"),
    media: query,
    ...matchMediaFillers
  })
});
...
describe("Styling utils", () => {
  describe("computeSpacingStyle", () => {
    test("pulls correct theme values", async () => {
      expect(computeSpacingStyle("sm md lg none", lightTheme.emotion)).toEqual("0.5rem 0.75rem 1rem 0");
      expect(computeSpacingStyle("xs  0  px  lg", lightTheme.emotion)).toEqual("0.375rem 0 1px 1rem");
    });
  });
});
describe("isPresetTheme", () => {
  it("returns true for the light, dark, and auto themes", () => {
    ...
    presetThemes.forEach(themeConfig => {
      expect(isPresetTheme(themeConfig)).toBe(true);
    });
  });
  it("returns false for custom themes", () => {
    const customTheme = createTheme(CUSTOM_THEME_NAME, new CustomThemeConfig({
      primaryColor: "red",
...
describe("Cached theme helpers", () => {
  ...
  const breakLocalStorage = () => {
    ...
    .spyOn(window.localStorage.__proto__, "setItem").mockImplementation(() => {
  ...
  describe("getCachedTheme", () => {
    ...
    it("does not find cached themes with older versions, so returns null", () => {
      ...
      window.localStorage.setItem(LocalStore.CACHED_THEME_BASE_KEY, JSON.stringify({
        name: darkTheme.name
      }));
      expect(getCachedTheme()).toBe(null);
    });
    it("returns preset cached theme if localStorage is available and one is set", () => {
      window.localStorage.setItem(LocalStore.ACTIVE_THEME, JSON.stringify({
        name: darkTheme.name
      }));
  ...
  describe("removeCachedTheme", () => {
    it("does nothing if localStorage is not available", () => {
      ...
      // eslint-disable-next-line no-proto
      window.localStorage.__proto__, "removeItem");
      removeCachedTheme();
  ...
  describe("setCachedTheme", () => {
    ...
    it("sets a preset theme with just its name if localStorage is available", () => {
      setCachedTheme(darkTheme);
      const cachedTheme = JSON.parse(window.localStorage.getItem(LocalStore.ACTIVE_THEME));
    ...
    it("deletes cached themes with older versions", () => {
      ...
      window.localStorage.setItem(LocalStore.CACHED_THEME_BASE_KEY, "I should get deleted too :|");
      setCachedTheme(customTheme);
      expect(window.localStorage.getItem("stActiveTheme")).toBe(null);
      expect(window.localStorage.getItem(LocalStore.CACHED_THEME_BASE_KEY)).toBe(null);
    });
...
describe("createTheme", () => {
  it("returns a theme", () => {
    ...
    expect(customTheme.emotion.genericFonts.bodyFont).toBe(lightTheme.emotion.fonts.serif);
    // If it is not provided, use the default
    expect(customTheme.emotion.colors.bgColor).toBe(lightTheme.emotion.colors.bgColor);
  });
  it("returns a theme based on a different theme", () => {
    ...
    const customTheme = createTheme(CUSTOM_THEME_NAME, customThemeConfig, darkTheme,
    // inSidebar
    true);
    expect(customTheme.name).toBe(CUSTOM_THEME_NAME);
    expect(customTheme.emotion.colors.primary).toBe("red");
    expect(customTheme.emotion.colors.secondaryBg).toBe("blue");
    expect(customTheme.emotion.genericFonts.bodyFont).toBe(darkTheme.emotion.fonts.serif);
    // If it is not provided, use the default
    expect(customTheme.emotion.colors.bgColor).toBe(darkTheme.emotion.colors.bgColor);
  ...
  it("handles hex values without #", () => {
    ...
    expect(customTheme.emotion.genericFonts.bodyFont).toBe(customTheme.emotion.fonts.serif);
    // If it is not provided, use the default
    expect(customTheme.emotion.colors.bgColor).toBe(darkTheme.emotion.colors.bgColor);
  });
  it("sets unspecified theme options using the given BaseTheme", () => {
    ...
    expect(customTheme.emotion.colors.warning).toBe(darkTheme.emotion.colors.warning);
  });
  it("sets auxiliary colors based on backgroundColor over the BaseTheme", () => {
    ...
    // user.
    expect(customTheme.emotion.colors.secondaryBg).toBe(lightTheme.emotion.colors.secondaryBg);
  });
});
...
describe("getDefaultTheme", () => {
  ...
  it("sets default to the light theme when an embed query parameter is set", () => {
    windowSpy = mockWindow(windowLocationSearch("?embed=true&embed_options=light_theme"));
    const defaultTheme = getDefaultTheme();
  ...
  it("sets default to the dark theme when an embed query parameter is set", () => {
    windowSpy = mockWindow(windowLocationSearch("?embed=true&embed_options=dark_theme"));
    const defaultTheme = getDefaultTheme();
    expect(defaultTheme.name).toBe("Dark");
  ...
  it("respects embed query parameter is set over system theme", () => {
    windowSpy = mockWindow(windowMatchMedia("dark"), windowLocationSearch("?embed=true&embed_options=light_theme"));
    const defaultTheme = getDefaultTheme();
    expect(defaultTheme.name).toBe("Light");
    // Also verify that the theme is our lightTheme.
    expect(defaultTheme.emotion.colors).toEqual(lightTheme.emotion.colors);
...
describe("createEmotionTheme", () => {
  ...
  it("defaults to base if missing value", () => {
    ...
    expect(theme.genericFonts.bodyFont).toBe(baseTheme.emotion.genericFonts.bodyFont);
    expect(theme.genericFonts.headingFont).toBe(baseTheme.emotion.genericFonts.headingFont);
    expect(theme.genericFonts.codeFont).toBe(baseTheme.emotion.genericFonts.codeFont);
  });
});
...
describe("converting font <> enum", () => {
  it("fontEnumToString converts to enum", () => {
    expect(fontEnumToString(CustomThemeConfig.FontFamily.SANS_SERIF)).toBe(fonts.sansSerif);
    expect(fontEnumToString(CustomThemeConfig.FontFamily.SERIF)).toBe(fonts.serif);
    expect(fontEnumToString(CustomThemeConfig.FontFamily.MONOSPACE)).toBe(fonts.monospace);
  });
  it("fontToEnum converts to string", () => {
    expect(fontToEnum(fonts.monospace)).toBe(CustomThemeConfig.FontFamily.MONOSPACE);
    expect(fontToEnum(fonts.sansSerif)).toBe(CustomThemeConfig.FontFamily.SANS_SERIF);
...
describe("hasLightBackgroundColor", () => {
  const testCases = [{
    ...
    theme: createTheme(CUSTOM_THEME_NAME, new CustomThemeConfig({
...
describe("getWrappedHeadersStyle", () => {
  ...
  headers.forEach(header => {
    it("test header '".concat(header, "'"), () => {
      const headerStyle = headersStyle["& ".concat(header)];
