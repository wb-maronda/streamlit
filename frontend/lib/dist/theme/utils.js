...
import { localStorageAvailable, LocalStore } from "../util/storageUtils";
import { isDarkThemeInQueryParams, isLightThemeInQueryParams } from "../util/utils";
...
function mergeTheme(theme, injectedTheme) {
...
export const getMergedLightTheme = once(() => mergeTheme(lightTheme, window.__streamlit?.LIGHT_THEME));
export const getMergedDarkTheme = once(() => mergeTheme(darkTheme, window.__streamlit?.DARK_THEME));
export const getSystemTheme = () => {
  return window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches ? getMergedDarkTheme() : getMergedLightTheme();
};
...
export const createPresetThemes = () => [createAutoTheme(), getMergedLightTheme(), getMergedDarkTheme()];
...
export const getHostSpecifiedTheme = () => {
