...
import HostCommunicationManager, { HOST_COMM_VERSION } from "./HostCommunicationManager";
...
function mockEventListeners() {
  ...
  return (type, event) => listeners[type].forEach(cb => cb(event));
}
describe("HostCommunicationManager messaging", () => {
  let hostCommunicationMgr;
  let dispatchEvent;
  let originalHash;
  let setAllowedOriginsFunc;
  let openCommFunc;
  let sendMessageToHostFunc;
  beforeEach(() => {
    ...
    setAllowedOriginsFunc = jest.spyOn(hostCommunicationMgr, "setAllowedOrigins");
    openCommFunc = jest.spyOn(hostCommunicationMgr, "openHostCommunication");
    sendMessageToHostFunc = jest.spyOn(hostCommunicationMgr, "sendMessageToHost");
  ...
  it("sets allowedOrigins properly & opens HostCommunication", () => {
    ...
    // @ts-expect-error
    expect(hostCommunicationMgr.allowedOrigins).toEqual(["https://devel.streamlit.test"]);
    expect(openCommFunc).toHaveBeenCalledWith();
  });
  it("host should receive a GUEST_READY message", () => {
    expect(sendMessageToHostFunc).toHaveBeenCalledWith({
      type: "GUEST_READY"
    });
  });
  it("can process a received CLOSE_MODAL message", () => {
    dispatchEvent("message", new MessageEvent("message", {
      data: {
  ...
  it("can process a received REQUEST_PAGE_CHANGE message", () => {
    ...
    // @ts-expect-error - props are private
    expect(hostCommunicationMgr.props.pageChanged).toHaveBeenCalledWith("hash1");
  });
  ...
  it("can process a received SET_INPUTS_DISABLED message", () => {
    ...
    // @ts-expect-error - props are private
    expect(hostCommunicationMgr.props.setInputsDisabled).toHaveBeenCalledWith(true);
  });
  it("should respond to SET_IS_OWNER message", () => {
    ...
    // @ts-expect-error - props are private
    expect(hostCommunicationMgr.props.isOwnerChanged).toHaveBeenCalledWith(true);
  });
  it("should respond to SET_MENU_ITEMS message", () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.hostMenuItemsChanged).toHaveBeenCalledWith([{
      type: "separator"
    }]);
  });
  it("should respond to SET_METADATA message", () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.deployedAppMetadataChanged).toHaveBeenCalledWith({
      hostedAt: "maya",
      owner: "corgi",
      repo: "streamlit"
    });
  ...
  it("can process a received SET_PAGE_LINK_BASE_URL message", () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.pageLinkBaseUrlChanged).toHaveBeenCalledWith("https://share.streamlit.io/vdonato/foo/bar");
  });
  it("can process a received SET_SIDEBAR_CHEVRON_DOWNSHIFT message", () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.sidebarChevronDownshiftChanged).toHaveBeenCalledWith(50);
  });
  it("can process a received SET_SIDEBAR_NAV_VISIBILITY message", () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.hostHideSidebarNavChanged).toHaveBeenCalledWith(true);
  });
  it("can process a received SET_TOOLBAR_ITEMS message", () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.hostToolbarItemsChanged).toHaveBeenCalledWith([{
      borderless: true,
      icon: "star.svg",
      key: "favorite",
      label: ""
  ...
  it("can process a received UPDATE_FROM_QUERY_PARAMS message", () => {
    ...
    // @ts-expect-error - props are private
    expect(hostCommunicationMgr.props.queryParamsChanged).toHaveBeenCalledWith("foo=bar");
    // @ts-expect-error - props are private
  ...
  it("can process a received SET_CUSTOM_THEME_CONFIG message", async () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.themeChanged).toHaveBeenCalledWith(undefined, mockCustomThemeConfig);
  });
  it("can process a received SET_CUSTOM_THEME_CONFIG message with a dark theme name", async () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.themeChanged).toHaveBeenCalledWith("Dark", undefined);
  });
  it("can process a received SET_CUSTOM_THEME_CONFIG message with a light theme name", async () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.themeChanged).toHaveBeenCalledWith("Light", undefined);
  });
  it("can process a received SET_AUTH_TOKEN message with JWT pair", () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.jwtHeaderChanged).toHaveBeenCalledWith(message.data);
  });
  it("can process a received RESTART_WEBSOCKET_CONNECTION message", () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.restartWebsocketConnection).toHaveBeenCalled();
  });
  it("can process a received TERMINATE_WEBSOCKET_CONNECTION message", () => {
    ...
    expect(
    // @ts-expect-error - props are private
    hostCommunicationMgr.props.terminateWebsocketConnection).toHaveBeenCalled();
  });
});
...
describe("HostCommunicationManager external auth token handling", () => {
  ...
  it("resolves promise to undefined immediately if useExternalAuthToken is false", async () => {
    const setAllowedOriginsFunc = jest.spyOn(hostCommunicationMgr, "setAllowedOrigins");
    hostCommunicationMgr.setAllowedOrigins({
    ...
    await expect(hostCommunicationMgr.deferredAuthToken.promise).resolves.toBe(undefined);
  });
  it("waits to receive SET_AUTH_TOKEN message before resolving promise if useExternalAuthToken is true", async () => {
    ...
    // @ts-expect-error - deferredAuthToken is private
    await expect(hostCommunicationMgr.deferredAuthToken.promise).resolves.toBe("i am an auth token");
    ...
    // @ts-expect-error - deferredAuthToken is private
    await expect(hostCommunicationMgr.deferredAuthToken.promise).resolves.toBe("i am a NEW auth token");
  });
});
//# sourceMappingURL=HostCommunicationManager.test.js.map