...
import { Dictionary, Null, Struct, tableFromIPC, util, Vector } from "apache-arrow";
...
const formatWeeks = (duration, freqParam) => {
  ...
  if (dayIndex < 0) {
    throw new Error(`Invalid value: ${freqParam}. Supported values: ${JSON.stringify(WEEKDAY_SHORT)}`);
  }
...
export class Quiver {
  ...
  getCategoricalOptions(dataColumnIndex) {
    ...
    const categoricalDict = this._data.getChildAt(dataColumnIndex)?.data[0]?.dictionary;
    if (categoricalDict) {
...
`);
  ...
  static sameDataTypes(t1, t2) {
    ...
    return t1.every((type, index) => type.pandas_type === t2[index]?.pandas_type);
  }
...
`);
  ...
  static formatDuration(data, field) {
    return moment.duration(Quiver.convertToSeconds(data, field?.type?.unit ?? 3), "seconds").humanize();
  }
  ...
  static formatPeriod(duration, freq) {
    ...
    if (!Number.isSafeInteger(durationNumber)) {
      logWarning(`Unsupported value: ${duration}. Supported values: [${Number.MIN_SAFE_INTEGER}-${Number.MAX_SAFE_INTEGER}]`);
      return String(duration);
    }
  ...
  static format(x, type, field) {
    ...
    if (typeName === "decimal") {
      return this.formatDecimal(x, field?.type?.scale || 0);
    }
   ...
   */
  get cssId() {
    if (isNullOrUndefined(this._styler) || isNullOrUndefined(this._styler.uuid)) {
      return undefined;
    }
  ...
  getCell(rowIndex, columnIndex) {
    ...
    if (isIndexCell) {
      ...
      const cssId = this._styler?.uuid ? `${this.cssId}level${columnIndex}_row${dataRowIndex}` : undefined;

      // Index label cells include:
      // - row_heading
      // - row<n> where n is the numeric position of the row
      // - level<k> where k is the level in a MultiIndex
      const cssClass = [`row_heading`, `level${columnIndex}`, `row${dataRowIndex}`].join(" ");
    ...
    if (isColumnsCell) {
      ...
      // - level<k> where k is the level in a MultiIndex
      const cssClass = [`col_heading`, `level${rowIndex}`, `col${dataColumnIndex}`].join(" ");
      return {
    ...
    const dataRowIndex = rowIndex - headerRows;
    const dataColumnIndex = columnIndex - headerColumns;
    const cssId = this._styler?.uuid ? `${this.cssId}row${dataRowIndex}_col${dataColumnIndex}` : undefined;

    // Data cells include `data`.
    const cssClass = ["data", `row${dataRowIndex}`, `col${dataColumnIndex}`].join(" ");
    ...
    const displayContent = this._styler?.displayValues ? this._styler.displayValues.getCell(rowIndex, columnIndex).content : undefined;
    return {
