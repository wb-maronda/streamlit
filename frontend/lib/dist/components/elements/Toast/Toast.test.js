...
import { Toast, shortenMessage } from "./Toast";
...
const createContainer = () => /*#__PURE__*/_jsx(ToasterContainer, {
...
const getProps = function () {
  ...
  let themeProps = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  return {
...
const renderComponent = props => render( /*#__PURE__*/_jsxs(_Fragment, {
  children: [createContainer(), /*#__PURE__*/_jsx(Toast, {
    ...props
  })]
}));
describe("Toast Component", () => {
  ...
  test("renders long toast messages with expand option", () => {
    ...
    expect(toast).toBeInTheDocument();
    expect(toastText).toHaveTextContent("Random toast message that is a really really really really really really really really really long");
  ...
  test("can expand to see the full toast message & collapse to truncate", () => {
    ...
    expect(toast).toHaveTextContent("Random toast message that is a really really really really really really really really really long message, going way past the 3 line limit");
  ...
  test("shortenMessage truncates messages over the character limit without cutting words", () => {
    var _screen$getByRole, _screen$getByRole$tex;
    const longMessage = "This is a very long message meant to test the functionality of the shortenMessage function, ensuring it truncates properly without cutting words and respects the character limit.";
    ...
    const toastText = (_screen$getByRole = screen.getByRole("alert")) === null || _screen$getByRole === void 0 ? void 0 : (_screen$getByRole$tex = _screen$getByRole.textContent) === null || _screen$getByRole$tex === void 0 ? void 0 : _screen$getByRole$tex.replace("view moreClose", "");
  ...
  test("shortenMessage handles explicit line breaks correctly", () => {
    var _screen$getByRole2, _screen$getByRole2$te;
    const messageWithBreaks = "First line of the message.\nSecond line of the message, which is meant to test how explicit line breaks are handled.\nThird line, which should not be visible.";
    const expectedTruncatedMessage = shortenMessage(messageWithBreaks);
    const props = getProps({
      icon: "",
      body: messageWithBreaks
    });
    renderComponent(props);
    const toastText = (_screen$getByRole2 = screen.getByRole("alert")) === null || _screen$getByRole2 === void 0 ? void 0 : (_screen$getByRole2$te = _screen$getByRole2.textContent) === null || _screen$getByRole2$te === void 0 ? void 0 : _screen$getByRole2$te.replace("view moreClose", "");
    expect(toastText).toEqual(expectedTruncatedMessage);
    expect(toastText).toHaveLength(expectedTruncatedMessage.length);
  });
  test("expands and collapses long messages with explicit line breaks correctly", () => {
    var _screen$getByRole3, _screen$getByRole3$te, _screen$getByRole4, _screen$getByRole4$te;
    ...
    const toastExpanded = (_screen$getByRole3 = screen.getByRole("alert")) === null || _screen$getByRole3 === void 0 ? void 0 : (_screen$getByRole3$te = _screen$getByRole3.textContent) === null || _screen$getByRole3$te === void 0 ? void 0 : _screen$getByRole3$te.replace("view lessClose", "");
    ...
    const toastCollapsed = (_screen$getByRole4 = screen.getByRole("alert")) === null || _screen$getByRole4 === void 0 ? void 0 : (_screen$getByRole4$te = _screen$getByRole4.textContent) === null || _screen$getByRole4$te === void 0 ? void 0 : _screen$getByRole4$te.replace("view moreClose", "");
