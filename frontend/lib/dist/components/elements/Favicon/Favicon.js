...
function iconToUrl(icon) {
  ...
  const iconUrl = "https://fonts.gstatic.com/s/i/short-term/release/materialsymbolsoutlined/".concat(matchResult[2], "/default/24px.svg");
  return iconUrl;
}
...
export function handleFavicon(favicon, sendMessageToHost, endpoints) {
  ...
  if (emoji && !favicon.startsWith(":material")) {
    ...
    const emojiUrl = "https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/72x72/".concat(codepoint, ".png");
    imageUrl = emojiUrl;
