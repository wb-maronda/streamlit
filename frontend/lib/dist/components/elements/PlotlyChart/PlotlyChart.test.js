...
import { applyTheming, handleSelection, parseBoxSelection, parseLassoPath, sendEmptySelection } from "./PlotlyChart";
...
describe("sendEmptySelection", () => {
  ...
  it("does not send a rerun msg if widget_state is empty", () => {
    ...
    widgetMgr.setStringValue(plotlyProto, '{"selection":{"points":[],"point_indices":[],"box":[],"lasso":[]}}', {
      fromUi: true
    }, undefined);
    sendEmptySelection(widgetMgr, {
      id: "plotly_chart"
...
describe("handleSelection", () => {
  ...
  it("should not rerun if lasso selection is present but has no lasso selection mode", () => {
    ...
    const widgetMgr = getWidgetMgr();
    jest.spyOn(widgetMgr, "setStringValue");
    handleSelection(event, widgetMgr,
    // @ts-expect-error
    {
      ...proto,
      selectionMode: []
  ...
  it("should not rerun if the return value is the same", () => {
    ...
    const widgetMgr = getWidgetMgr();
    jest.spyOn(widgetMgr, "setStringValue");
    widgetMgr.setStringValue(proto, '{"selection":{"points":[],"point_indices":[],"box":[],"lasso":[]}}', {
      fromUi: true
    }, undefined);
    handleSelection(event, widgetMgr, proto, mockFragmentId);
    expect(widgetMgr.setStringValue).toHaveBeenCalledTimes(1);
  ...
  it('should rerun if there is a lasso select and a box select when selection_mode=["box", "lasso"]', () => {
    ...
    const widgetMgr = getWidgetMgr();
    jest.spyOn(widgetMgr, "setStringValue");
    handleSelection(boxEvent, widgetMgr, {
      ...proto,
      selectionMode: [1, 2]
    }, undefined);
    expect(widgetMgr.setStringValue).toHaveBeenCalledTimes(1);
    ...
    handleSelection(lassoEventAndBoxEvent, widgetMgr, {
      ...proto,
      selectionMode: [1, 2]
    }, undefined);
