...
import { getDecreasingRed, getGray30, getGray70, getGray90, getIncreasingGreen, hasLightBackgroundColor, getSequentialColorsArray, getCategoricalColorsArray, getDivergingColorsArray } from "../../../theme";
 ...
 */
export function applyStreamlitThemeTemplateLayout(layout, theme) {
  const {
    genericFonts,
    colors,
    fontSizes
...
function replaceGOSpecificColors(spec, theme) {
  ...
  spec = spec.replaceAll(TOTAL, hasLightBackgroundColor(theme) ? theme.colors.blue80 : theme.colors.blue40);
...
export function replaceTemporaryColors(spec, theme, elementTheme) {
 ...
 * @param spec - spec
 */
export function applyStreamlitTheme(spec, theme) {
  try {
  ...
  if ("title" in spec.layout) {
    spec.layout.title = merge(spec.layout.title, {
      text: "<b>".concat(spec.layout.title.text, "</b>")
    });
  }
}
 ...
 */
export function layoutWithThemeDefaults(layout, theme) {
  const {
    colors,
    genericFonts
  } = theme;
