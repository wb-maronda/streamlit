...
import { keysToSnakeCase, notNullOrUndefined } from "../../../util/utils";
import { FormClearHelper } from "../../widgets/Form/FormClearHelper";
import { applyStreamlitTheme, layoutWithThemeDefaults, replaceTemporaryColors } from "./CustomTheme";
 ...
 */
export function parseLassoPath(pathData) {
  if (pathData === "") {
    return {
      x: [],
      y: []
 ...
 * @returns {SelectionRange} An object containing two arrays: `x` for all x coordinates and `y` for all y coordinates.
 */
export function parseBoxSelection(selection) {
  const hasRequiredFields = "x0" in selection && "x1" in selection && "y0" in selection && "y1" in selection;
  ...
  const x = [selection.x0, selection.x1];
  const y = [selection.y0, selection.y1];
  return {
    x,
    y
  };
 ...
 */
export function applyTheming(plotlyFigure, chartTheme, theme) {
  const spec = JSON.parse(replaceTemporaryColors(JSON.stringify(plotlyFigure), theme, chartTheme));
 ...
 */
export function handleSelection(event, widgetMgr, element, fragmentId) {
  if (!event) {
    return;
  }
  ...
  if (points) {
    points.forEach(function (point) {
      ...
      if (notNullOrUndefined(point.pointIndices) && point.pointIndices.length > 0) {
        point.pointIndices.forEach(item => selectedPointIndices.add(item));
      }
    });
  }
  if (selections) {
  ...
  selectionState.selection.points = selectedPoints.map(point => keysToSnakeCase(point));
  selectionState.selection.box = selectedBoxes;
  selectionState.selection.lasso = selectedLassos;
  if (selectionState.selection.box.length > 0 && !element.selectionMode.includes(PlotlyChartProto.SelectionMode.BOX)) {
    // If box selection is not activated, we don't want
    // to send any box selection related updates to the frontend
    return;
  }
  if (selectionState.selection.lasso.length > 0 && !element.selectionMode.includes(PlotlyChartProto.SelectionMode.LASSO)) {
  ...
  if (currentSelectionState !== newSelectionState) {
    ...
    widgetMgr.setStringValue(element, newSelectionState, {
      fromUi: true
    }, fragmentId);
  }
}
 ...
 * @param fragmentId The fragment id
 */
export function sendEmptySelection(widgetMgr, element, fragmentId) {
...
export function PlotlyChart(_ref) {
  var _plotlyFigure$layout, _initialFigureSpec$la, _plotlyFigure$layout5, _plotlyFigure$frames, _plotlyFigure$layout6;
  let {
    element,
    width,
    height,
  ...
  const initialFigureSpec = useMemo(() => {
    if (!element.spec) {
      return {
        layout: {},
        data: [],
  ...
  const isLassoSelectionActivated = isSelectionActivated && element.selectionMode.includes(PlotlyChartProto.SelectionMode.LASSO);
  const isBoxSelectionActivated = isSelectionActivated && element.selectionMode.includes(PlotlyChartProto.SelectionMode.BOX);
  const isPointsSelectionActivated = isSelectionActivated && element.selectionMode.includes(PlotlyChartProto.SelectionMode.POINTS);
  const plotlyConfig = useMemo(() => {
    ...
    if (!disableFullscreenMode) {
      ...
      config.modeBarButtonsToAdd = [{
        ...
        icon: isFullScreen ? FULLSCREEN_COLLAPSE_ICON : FULLSCREEN_EXPAND_ICON,
        click: () => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [element.id, element.config, isFullScreen, disableFullscreenMode, isSelectionActivated, isLassoSelectionActivated, isBoxSelectionActivated, collapse, expand]);
  useEffect(() => {
    // If the theme changes, we need to reapply the theming to the figure
    setPlotlyFigure(prevState => {
      return applyTheming(prevState, element.theme, theme);
    });
  }, [element.id, theme, element.theme]);
  useEffect(() => {
    let updatedClickMode = initialFigureSpec.layout.clickmode;
    let updatedHoverMode = initialFigureSpec.layout.hovermode;
    let updatedDragMode = initialFigureSpec.layout.dragmode;
    if (disabled) {
    ...
    setPlotlyFigure(prevState => {
      if (prevState.layout.clickmode === updatedClickMode && prevState.layout.hovermode === updatedHoverMode && prevState.layout.dragmode === updatedDragMode) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [element.id, isSelectionActivated, isPointsSelectionActivated, isBoxSelectionActivated, isLassoSelectionActivated, disabled]);
  let calculatedWidth = width === -1 ? // In some situations - e.g. initial loading of tabs - the width is set to -1
  // before its able to determine the real width. We want to keep the previous
  // width in this case.
  (_plotlyFigure$layout = plotlyFigure.layout) === null || _plotlyFigure$layout === void 0 ? void 0 : _plotlyFigure$layout.width : Math.max(element.useContainerWidth ? width : Math.min((_initialFigureSpec$la = initialFigureSpec.layout.width) !== null && _initialFigureSpec$la !== void 0 ? _initialFigureSpec$la : width, width),
  ...
  if (plotlyFigure.layout.height !== calculatedHeight || plotlyFigure.layout.width !== calculatedWidth) {
   ...
   */
  const handleSelectionCallback = useCallback(event => {
   ...
   */
  const resetSelectionsCallback = useCallback(function () {
    let resetSelectionInFigure = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
  ...
  useEffect(() => {
    ...
    formClearHelper.manageFormClearListener(widgetMgr, element.formId, resetSelectionsCallback);
    return () => {
      formClearHelper.disconnect();
    };
  }, [element.formId, widgetMgr, isSelectionActivated, resetSelectionsCallback]);
  useEffect(() => {
    var _plotlyFigure$layout2, _plotlyFigure$layout3, _plotlyFigure$layout4;
    if (!isSelectionActivated) {
      return;
    }
    ...
    let clickmode;
    if (((_plotlyFigure$layout2 = plotlyFigure.layout) === null || _plotlyFigure$layout2 === void 0 ? void 0 : _plotlyFigure$layout2.dragmode) === "select" || ((_plotlyFigure$layout3 = plotlyFigure.layout) === null || _plotlyFigure$layout3 === void 0 ? void 0 : _plotlyFigure$layout3.dragmode) === "lasso") {
      clickmode = "event";
    } else {
    ...
    if (((_plotlyFigure$layout4 = plotlyFigure.layout) === null || _plotlyFigure$layout4 === void 0 ? void 0 : _plotlyFigure$layout4.clickmode) !== clickmode) {
      setPlotlyFigure(prevFigure => {
        return {
          ...prevFigure,
          layout: {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [(_plotlyFigure$layout5 = plotlyFigure.layout) === null || _plotlyFigure$layout5 === void 0 ? void 0 : _plotlyFigure$layout5.dragmode]);
  return /*#__PURE__*/_jsx(Plot, {
    className: "stPlotlyChart",
    data: plotlyFigure.data,
    layout: plotlyFigure.layout,
    config: plotlyConfig,
    frames: (_plotlyFigure$frames = plotlyFigure.frames) !== null && _plotlyFigure$frames !== void 0 ? _plotlyFigure$frames : undefined,
    style: {
      // Hide the plotly chart if the width is not defined yet
      // to prevent flickering issues.
      visibility: ((_plotlyFigure$layout6 = plotlyFigure.layout) === null || _plotlyFigure$layout6 === void 0 ? void 0 : _plotlyFigure$layout6.width) === undefined ? "hidden" : undefined
    },
    ...
    // sometimes.
    ,
    onDoubleClick: isSelectionActivated ? () => resetSelectionsCallback() : undefined,
    onDeselect: isSelectionActivated ? () => {
