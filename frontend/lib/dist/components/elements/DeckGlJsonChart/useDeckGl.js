...
import { useStWidthHeight } from "../../../hooks/useStWidthHeight";
import { useBasicWidgetClientState } from "../../../useBasicWidgetState";
import { jsonConverter } from "./utils/jsonConverter";
import { getContextualFillColor, LAYER_TYPE_TO_FILL_FUNCTION } from "./utils/colors";
...
function getDefaultState(widgetMgr, element) {
  if (!element.id) {
    return EMPTY_STATE;
  }
...
function getStateFromWidgetMgr(widgetMgr, element) {
  ...
  const currState = stringValue ? JSON5.parse(stringValue) : null;
...
function updateWidgetMgrState(element, widgetMgr, vws, fragmentId) {
  ...
  widgetMgr.setStringValue(element, JSON.stringify(vws.value), {
    fromUi: vws.fromUi
  }, fragmentId);
}
export const useDeckGl = props => {
  const {
    element,
    fragmentId,
    height: propsHeight,
  ...
  const {
    ...
    heightFallback: viewState.initialViewState?.height || DEFAULT_DECK_GL_HEIGHT
  });
   ...
   */
  const selectionMode = allSelectionModes[0];
  const isSelectionModeActivated = selectionMode !== undefined;
  const hasActiveSelection = isSelectionModeActivated && Object.keys(data.selection.indices).length > 0;
  const parsedPydeckJson = useMemo(() => {
    return Object.freeze(JSON5.parse(element.json));
  ...
  const deck = useMemo(() => {
    ...
    if (!copy.mapStyle) {
      copy.mapStyle = `mapbox://styles/mapbox/${isLightTheme ? "light" : "dark"}-v9`;
    }

    // Set width and height based on the fullscreen state
    if (isFullScreen) {
      Object.assign(copy.initialViewState, {
        width,
        height
      });
    } else {
    ...
    if (copy.layers) {
      ...
      const anyLayersHavePickableDefined = copy.layers.some(layer => Object.hasOwn(layer, "pickable"));
      copy.layers = copy.layers.map(layer => {
        ...
        fillFunctions.forEach(fillFunction => {
          clonedLayer.updateTriggers = {
            ...
            [fillFunction]: [...(clonedLayer.updateTriggers?.[fillFunction] || []), selectedIndices, anyLayersHaveSelection]
          };
          ...
          const originalFillFunction = layer[fillFunction];
          ...
          const newFillFunction = (object, objectInfo) => {
  ...
  }, [data.selection.indices, height, isFullScreen, isLightTheme, isSelectionModeActivated, parsedPydeckJson, shouldUseContainerWidth, theme.colors.gray20, theme.colors.primary, width]);
  useEffect(() => {
  ...
  const createTooltip = useCallback(info => {
  ...
  const onViewStateChange = useCallback(_ref => {
    let {
      viewState
    } = _ref;
...
//# sourceMappingURL=useDeckGl.js.map