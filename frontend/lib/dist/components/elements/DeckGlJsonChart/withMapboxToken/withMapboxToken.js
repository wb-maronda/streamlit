...
const withMapboxToken = deltaType => WrappedComponent => {
    ...
    constructor(props) {
      ...
      this.initMapboxToken = async () => {
        try {
          ...
          if (!token) {
            throw new Error("Missing token ".concat(MAPBOX));
          }
          this.setState({
            mapboxToken: token,
            isFetching: false
          ...
          throw new MapboxTokenFetchingError("".concat(error.message, " (").concat(TOKENS_URL, ")"));
        }
      };
      this.render = () => {
        const {
  ...
  WithMapboxToken.displayName = "withMapboxToken(".concat(WrappedComponent.displayName || WrappedComponent.name, ")");
