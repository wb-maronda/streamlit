...
const getProps = function () {
  ...
  let initialViewStateProps = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  const json = {
...
const getUseDeckGlProps = function () {
  let elementProps = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  let initialViewStateProps = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  return {
...
describe("DeckGlJsonChart element", () => {
  it("renders without crashing", () => {
    ...
    render( /*#__PURE__*/_jsx(DeckGlJsonChart, {
      ...props
    }));
...
describe("#useDeckGl", () => {
  it("should merge client and server changes in viewState", () => {
    ...
    rerender({
      ...initialProps,
      element: getUseDeckGlProps({}, {
        zoom: 8
      }).element
    });

    // should match original mockInitialViewState
    expect(current.viewState).toEqual({
  ...
  describe("createTooltip", () => {
    ...
    it("should return null if info.object is undefined", () => {
      ...
      expect(current.createTooltip({})).toBe(null);
    });
  ...
  describe("deck", () => {
    ...
    const testCases = [{
      ...
      newProps: getUseDeckGlProps(undefined, {
        zoom: 19
      })
    }, {
    ...
    it.each(testCases)("$description", _ref => {
      let {
        newProps
      } = _ref;
      const initialProps = getUseDeckGlProps();
  ...
  describe("selectionMode", () => {
    ...
    it("should be defined when allSelectionModes has single object select", () => {
      ...
      expect(result.current.selectionMode).toBe(DeckGlJsonChartProto.SelectionMode.SINGLE_OBJECT);
    });
    it("should be defined when allSelectionModes has multi object select", () => {
      ...
      expect(result.current.selectionMode).toBe(DeckGlJsonChartProto.SelectionMode.MULTI_OBJECT);
    });
    it("should return the first selection mode given, if multiple are given", () => {
      const initialProps = getUseDeckGlProps({
        selectionMode: [DeckGlJsonChartProto.SelectionMode.MULTI_OBJECT, DeckGlJsonChartProto.SelectionMode.SINGLE_OBJECT]
      });
      const {
        result
      } = renderHook(props => useDeckGl(props), {
      ...
      await act(async () => {
        result.current.setSelection({
          ...
          value: {
            selection: {
              indices: {
                "0533490f-fcf9-4dc0-8c94-ae4fbd42eb6f": [0]
              },
              objects: {
                "0533490f-fcf9-4dc0-8c94-ae4fbd42eb6f": [{}]
              }
            }
          }
        });
...
//# sourceMappingURL=DeckGlJsonChart.test.js.map