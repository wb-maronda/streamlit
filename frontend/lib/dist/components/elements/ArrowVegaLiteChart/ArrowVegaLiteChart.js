...
import { debounce, isNullOrUndefined, notNullOrUndefined } from "../../../util/utils";
import { logMessage, logWarning } from "../../../util/log";
...
import { dataIsAnAppendOfPrev, getDataArray, getDataArrays, getDataSets, getInlineData } from "./arrowUtils";
  ...
  constructor() {
    ...
    this.maybeConfigureSelections = () => {
      ...
      element.selectionMode.forEach((param, _index) => {
        this.vegaView?.addSignalListener(param, debounce(DEBOUNCE_TIME_MS, (name, value) => {
          ...
          const viewState = this.vegaView?.getState({
            ...
            data: (name, _operator) => {
              ...
              return element.selectionMode.some(mode => `${mode}_store` === name);
            },
      ...
      const reset = () => {
        ...
        if (!isEqual(currentWidgetState, emptySelectionState)) {
          this.props.widgetMgr?.setStringValue(this.props.element, JSON.stringify(emptySelectionState), {
            fromUi: true
          }, this.props.fragmentId);
        }
      };
