...
import { CompactSelection, DataEditor as GlideDataEditor } from "@glideapps/glide-data-grid";
import { Resizable } from "re-resizable";
import { Add, Close, Delete, FileDownload, Search } from "@emotion-icons/material-outlined";
...
import { useColumnLoader, useColumnSizer, useColumnSort, useCustomRenderer, useCustomTheme, useDataEditor, useDataExporter, useDataLoader, useSelectionHandler, useTableSizer, useTooltips } from "./hooks";
import { BORDER_THRESHOLD, MAX_COLUMN_AUTO_WIDTH, MAX_COLUMN_WIDTH, MIN_COLUMN_WIDTH, ROW_HEIGHT } from "./hooks/useTableSizer";
  ...
  React.useEffect(() => {
    ...
    setTimeout(() => {
      if (resizableContainerRef.current && dataEditorRef.current) {
        ...
        const scrollAreaBounds = resizableContainerRef.current?.querySelector(".dvn-stack")?.getBoundingClientRect();
