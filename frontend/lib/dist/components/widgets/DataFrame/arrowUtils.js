...
import { ObjectColumn, CheckboxColumn, NumberColumn, TextColumn, SelectboxColumn, ListColumn, isErrorCell, DateTimeColumn, TimeColumn, DateColumn, removeLineBreaks } from "./columns";
 ...
 */
export function extractCssProperty(htmlElementId, property, cssStyle) {
  // This regex is supposed to extract the value of a CSS property
  // for a specified HTML element ID from a CSS style string:
  const regex = new RegExp("".concat(htmlElementId, "[,\\s].*{(?:[^}]*[\\s;]{1})?").concat(property, ":\\s*([^;}]+)[;]?.*}"), "gm");
 ...
 */
export function applyPandasStylerCss(cell, cssId, cssStyles) {
  const themeOverride = {};
 ...
 */
export function getColumnTypeFromArrow(arrowType) {
  ...
  if (["int8", "int16", "int32", "int64", "uint8", "uint16", "uint32", "uint64", "float16", "float32", "float64", "float96", "float128", "range",
  // The default index in pandas uses a range type.
  "decimal"].includes(typeName)) {
    return NumberColumn;
  }
 ...
 */
export function getIndexFromArrow(data, indexPosition) {
 ...
 */
export function getColumnFromArrow(data, columnPosition) {
  ...
  return {
    id: "column-".concat(title, "-").concat(columnPosition),
    name: title,
    title,
    isEditable: true,
    arrowType,
 ...
 */
export function getEmptyIndexColumn() {
  return {
    id: "empty-index",
    title: "",
    indexNumber: 0,
 ...
 */
export function getAllColumnsFromArrow(data) {
  const columns = [];
  const {
    dimensions
  } = data;
 ...
 */
export function getCellFromArrow(column, arrowCell) {
  let cssStyles = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
  const typeName = column.arrowType ? Quiver.getTypeName(column.arrowType) : null;
  let cellTemplate;
  if (column.kind === "object") {
    ...
    cellTemplate = column.getCell(notNullOrUndefined(arrowCell.content) ? removeLineBreaks(Quiver.format(arrowCell.content, arrowCell.contentType, arrowCell.field)) : null);
  } else if (["time", "date", "datetime"].includes(column.kind) && notNullOrUndefined(arrowCell.content) && (typeof arrowCell.content === "number" || typeof arrowCell.content === "bigint")) {
    var _arrowCell$field, _arrowCell$field$type;
    ...
    let parsedDate;
    if (typeName === "time" && notNullOrUndefined((_arrowCell$field = arrowCell.field) === null || _arrowCell$field === void 0 ? void 0 : (_arrowCell$field$type = _arrowCell$field.type) === null || _arrowCell$field$type === void 0 ? void 0 : _arrowCell$field$type.unit)) {
      var _arrowCell$field$type2, _arrowCell$field2, _arrowCell$field2$typ;
      // Time values needs to be adjusted to seconds based on the unit
      parsedDate = moment.unix(Quiver.convertToSeconds(arrowCell.content, (_arrowCell$field$type2 = (_arrowCell$field2 = arrowCell.field) === null || _arrowCell$field2 === void 0 ? void 0 : (_arrowCell$field2$typ = _arrowCell$field2.type) === null || _arrowCell$field2$typ === void 0 ? void 0 : _arrowCell$field2$typ.unit) !== null && _arrowCell$field$type2 !== void 0 ? _arrowCell$field$type2 : 0)).utc().toDate();
    } else {
    ...
    const decimalStr = isNullOrUndefined(arrowCell.content) ? null : Quiver.format(arrowCell.content, arrowCell.contentType, arrowCell.field);
  ...
  if (!column.isEditable) {
    ...
    if (notNullOrUndefined(arrowCell.displayContent)) {
      ...
      } else if (cellTemplate.kind === GridCellKind.Custom && ((_data = cellTemplate.data) === null || _data === void 0 ? void 0 : _data.kind) === "date-picker-cell") {
        cellTemplate = {
          ...cellTemplate,
          data: {
            ...cellTemplate.data,
