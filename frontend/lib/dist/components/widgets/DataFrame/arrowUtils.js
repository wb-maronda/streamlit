...
import { CheckboxColumn, DateColumn, DateTimeColumn, isErrorCell, ListColumn, NumberColumn, ObjectColumn, removeLineBreaks, SelectboxColumn, TextColumn, TimeColumn } from "./columns";
...
export function getCellFromArrow(column, arrowCell) {
  ...
  if (column.kind === "object") {
    ...
    let parsedDate;
    if (typeName === "time" && notNullOrUndefined(arrowCell.field?.type?.unit)) {
      // Time values needs to be adjusted to seconds based on the unit
      parsedDate = moment.unix(Quiver.convertToSeconds(arrowCell.content, arrowCell.field?.type?.unit ?? 0)).utc().toDate();
    } else {
  ...
  if (!column.isEditable) {
    ...
    if (notNullOrUndefined(arrowCell.displayContent)) {
      ...
      } else if (cellTemplate.kind === GridCellKind.Custom && cellTemplate.data?.kind === "date-picker-cell") {
        cellTemplate = {
          ...cellTemplate,
          data: {
            ...cellTemplate.data,
