...
import { isMissingValueCell } from "./columns";
...
class EditingState {
  ...
  toJson(columns) {
    ...
    this.addedRows.forEach(row => {
      ...
      row.forEach((cell, colIndex, _map) => {
        ...
        if (column) {
          ...
          if (column.isRequired && column.isEditable && isMissingValueCell(cell)) {
    ...
    const json = JSON.stringify(currentState, (_k, v) => v === undefined ? null : v);
    return json;
  }
  ...
  fromJson(editingStateJson, columns) {
    ...
    Object.keys(editingState.edited_rows).forEach(key => {
      ...
      Object.keys(editedRow).forEach(colName => {
        ...
        if (column) {
          ...
          if (cell) {
            ...
            (_this$editedCells$get = this.editedCells.get(rowIndex)) === null || _this$editedCells$get === void 0 ? void 0 : _this$editedCells$get.set(column.indexNumber, cell);
          }
        }
      });
    });
