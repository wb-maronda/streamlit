...
import { UNICODE, MULTI, CATEGORICAL_COLUMN, DECIMAL, STYLER, DISPLAY_VALUES, EMPTY } from "../../../mocks/arrow";
import { getTextCell, ObjectColumn, CheckboxColumn, NumberColumn, TextColumn, SelectboxColumn, ListColumn, DateTimeColumn, DateColumn, TimeColumn } from "./columns";
import { extractCssProperty, applyPandasStylerCss, getColumnTypeFromArrow, getIndexFromArrow, getColumnFromArrow, getAllColumnsFromArrow, getCellFromArrow } from "./arrowUtils";
...
describe("extractCssProperty", () => {
  it("should extract the correct property value", () => {
    ...
    expect(extractCssProperty("#T_f116e_row10_col1", "color", cssStyle1)).toBe("red");
    expect(extractCssProperty("#T_f116e_row12_col1", "background-color", cssStyle1)).toBe("pink");
    expect(extractCssProperty("#T_f116e_row5_col1", "color", cssStyle1)).toBe(undefined);
    expect(extractCssProperty("foo", "color", cssStyle1)).toBe(undefined);
    expect(extractCssProperty("#T_f116e_row0_col2", "color", cssStyle1)).toBe("red");
    expect(extractCssProperty("#T_f116e_row11_col10", "background-color", cssStyle1)).toBe("darkblue");
    // Should not extract if it only partly matches:
    expect(extractCssProperty("#T_f116e_row11_col1", "background-color", cssStyle1)).toBe(undefined);
    expect(extractCssProperty("#T_7e5cc_row6_col0", "background-color", cssStyle2)).toBe("#f8fcc9");
    expect(extractCssProperty("#T_7e5cc_row9_col0", "color", cssStyle2)).toBe("#000000");
    expect(extractCssProperty("#T_f116e_row12_col14", "background-color", cssStyle2)).toBe("blue");
    expect(extractCssProperty("#T_f116e_row13_col14", "background-color", cssStyle2)).toBe("#f1f1f1");
    expect(extractCssProperty("#T_f116e_row14_col1", "background-color", cssStyle2)).toBe("rgba(72 122 180 / .2)");
    expect(extractCssProperty("#T_f116e_row15_col1", "background-color", cssStyle2)).toBe("rgba(255, 0, 12, .2)");
    expect(extractCssProperty("#T_f116e_row16_col14", "background-color", cssStyle2)).toBe("hsla(240, 100%, 90%)");
    expect(extractCssProperty("#T_f116e_row17_col1", "background-color", cssStyle2)).toBe("hsl(255, 0, 12)");
    expect(extractCssProperty("#T_f116e_row10_col0", "background-color", cssStyle3)).toBe("#f8fcc9");
    expect(extractCssProperty("#T_7e5cc_row8_col0", "background-color", cssStyle3)).toBe("#289cc1");
    expect(extractCssProperty("#T_f116e_row18_col14", "background-color", cssStyle3)).toBe("hsla(240, 100%,    90%)");
    expect(extractCssProperty("#T_f116e_row19_col14", "background-color", cssStyle3)).toBe("hsl(240, 100%,90%)");
    expect(extractCssProperty("#T_7e5cc_row8_col0", "color", cssStyle3)).toBe("#f1f1f1");
  });
});
describe("applyPandasStylerCss", () => {
  it("should apply css to a cells", () => {
    ...
    let styledCell = applyPandasStylerCss(MOCK_CELL, "#T_f116e_row11_col10", CSS_STYLES);
    expect(styledCell.themeOverride).toEqual({
      bgCell: "darkblue"
    });
    styledCell = applyPandasStylerCss(MOCK_CELL, "#T_f116e_row0_col2", CSS_STYLES);
    expect(styledCell.themeOverride).toEqual({
      textDark: "red"
    });
    styledCell = applyPandasStylerCss(MOCK_CELL, "#T_f116e_row3_col3", CSS_STYLES);
    expect(styledCell.themeOverride).toEqual({
      bgCell: "pink",
      textDark: "white"
    });
  ...
  it("should use a grey color when background is yellow", () => {
    ...
    const styledCell = applyPandasStylerCss(getTextCell(true, false), "#T_f116e_row0_col0", CSS_STYLES);
    expect(styledCell.themeOverride).toEqual({
      bgCell: "yellow",
      textDark: "#31333F"
    });
...
describe("getCellFromArrow", () => {
  ...
  it("applies display content overwrite to time cells", () => {
    ...
    expect(cell.data.displayDate).toEqual("FOOO");
  });
  it("parses numeric timestamps for time columns into valid Date values", () => {
    ...
    expect(MOCK_TIME_COLUMN.getCell).toHaveBeenCalledWith(new Date("2021-09-29T21:13:20.123Z"));
  });
  ...
  it("applies Pandas styler CSS", () => {
    const element = {
      ...
      styler: {
        uuid: "FAKE_UUID",
        styles: "#T_FAKE_UUIDrow1_col1, #T_FAKE_UUIDrow0_col0 { color: white; background-color: pink }",
        displayValues: DISPLAY_VALUES,
        caption: "FAKE_CAPTION"
      }
    };
    const data = new Quiver(element);
    const cell = getCellFromArrow(MOCK_NUMBER_COLUMN, data.getCell(1, 1), element.styler.styles);
    expect(cell).toEqual({
      allowOverlay: true,
      contentAlign: "right",
      copyData: "1",
...
it("doesn't apply Pandas Styler CSS for editable columns", () => {
  ...
  const cell = getCellFromArrow({
    ...MOCK_NUMBER_COLUMN,
    isEditable: true
  }, data.getCell(1, 1), element.styler.styles);
  expect(cell).toEqual({
    allowOverlay: true,
    contentAlign: "right",
    copyData: "1",
...
describe("getColumnTypeFromArrow", () => {
  it.each([[{
    ...
    pandas_type: "bytes",
    numpy_type: "object"
  }, ObjectColumn]])("interprets %p as column type: %p", (arrowType, expectedType) => {
...
describe("isIntegerType", () => {
  it.each([[{
    ...
    pandas_type: "object",
    numpy_type: "interval[int64, both]"
  }, false]])("interprets %p as integer type: %p", (arrowType, expected) => {
    expect(isIntegerType(Quiver.getTypeName(arrowType))).toEqual(expected);
  });
});
//# sourceMappingURL=arrowUtils.test.js.map