...
import useTooltips, { DEBOUNCE_TIME_MS, REQUIRED_CELL_TOOLTIP } from "./useTooltips";
...
const getCellContentMock = jest.fn().mockImplementation(_ref => {
  ...
  if (column.kind === "number") {
    return {
      ...column.getCell(123),
      tooltip: "Cell tooltip 1"
    };
  }
  return {
    ...column.getCell("foo"),
    tooltip: "Cell tooltip 2"
  };
});
const getEmptyCellContentMock = jest.fn().mockImplementation(_ref2 => {
  let [col] = _ref2;
  const column = MOCK_COLUMNS[col];
...
//# sourceMappingURL=useTooltips.test.js.map