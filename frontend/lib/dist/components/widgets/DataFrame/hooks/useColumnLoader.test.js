...
import { CheckboxColumn, ListColumn, NumberColumn, ObjectColumn, SelectboxColumn, TextColumn } from "../columns";
import useColumnLoader, { applyColumnConfig, COLUMN_POSITION_PREFIX, COLUMN_WIDTH_MAPPING, getColumnConfig, getColumnType, INDEX_IDENTIFIER } from "./useColumnLoader";
...
describe("applyColumnConfig", () => {
  ...
  it("allows configuring a column via numeric ID", () => {
    const columnConfig = new Map([[`${COLUMN_POSITION_PREFIX}0`, {
      width: "small"
    }]]);
    const column1 = applyColumnConfig(MOCK_COLUMNS[0], columnConfig);
    expect(column1.width).toBe(COLUMN_WIDTH_MAPPING.small);
