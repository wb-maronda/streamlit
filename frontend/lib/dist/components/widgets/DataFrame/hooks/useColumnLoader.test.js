...
import { ObjectColumn, TextColumn, CheckboxColumn, SelectboxColumn, ListColumn, NumberColumn } from "../columns";
import useColumnLoader, { applyColumnConfig, getColumnConfig, getColumnType, INDEX_IDENTIFIER, COLUMN_POSITION_PREFIX, COLUMN_WIDTH_MAPPING } from "./useColumnLoader";
...
describe("applyColumnConfig", () => {
  it("should correctly apply the use-defined column config", () => {
    const columnConfig = new Map([["column_1", {
      width: "small",
      disabled: false,
      type_config: {
        type: "text"
  ...
  it("allows configuring the index via `index` as ID", () => {
    const columnConfig = new Map([[INDEX_IDENTIFIER, {
      width: "small"
    }]]);
  ...
  it("allows configuring a column via numeric ID", () => {
    const columnConfig = new Map([["".concat(COLUMN_POSITION_PREFIX, "0"), {
      width: "small"
    }]]);
    const column1 = applyColumnConfig(MOCK_COLUMNS[0], columnConfig);
    expect(column1.width).toBe(COLUMN_WIDTH_MAPPING.small);
  ...
  it("works with empty column configs", () => {
    const emptyColumnConfig = new Map([]);
...
describe("getColumnType", () => {
  ...
  it.each([["object", ObjectColumn], ["text", TextColumn], ["checkbox", CheckboxColumn], ["selectbox", SelectboxColumn], ["list", ListColumn], ["number", NumberColumn]])("maps user-specified type to column type (%p parsed as %p)", (typeName, columnCreator) => {
...
//# sourceMappingURL=useColumnLoader.test.js.map