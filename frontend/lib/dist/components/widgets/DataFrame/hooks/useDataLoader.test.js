...
import { isErrorCell, TextColumn } from "../columns";
...
describe("useDataLoader hook", () => {
  it("creates a glide-data-grid compatible callback to access cells", () => {
    ...
    const {
      ...
      const editingState = React.useRef(new EditingState(numRows));
      return useDataLoader(data, MOCK_COLUMNS, numRows, editingState);
    });

    // Row 1
    expect(MOCK_COLUMNS[0].getCellValue(result.current.getCellContent([0, 0]))).toBe("i1");
    expect(MOCK_COLUMNS[1].getCellValue(result.current.getCellContent([1, 0]))).toBe("foo");
    expect(MOCK_COLUMNS[2].getCellValue(result.current.getCellContent([2, 0]))).toBe("1");

    // Row 2
    expect(MOCK_COLUMNS[0].getCellValue(result.current.getCellContent([0, 1]))).toBe("i2");
    expect(MOCK_COLUMNS[1].getCellValue(result.current.getCellContent([1, 1]))).toBe("bar");
    expect(MOCK_COLUMNS[2].getCellValue(result.current.getCellContent([2, 1]))).toBe("2");
  ...
  it("uses editing state if a cell got edited", () => {
    ...
    expect(MOCK_COLUMNS[1].getCellValue(result.current.getCellContent([1, 0]))).toEqual("edited");
  });
  it("uses editing state if a row got deleted", () => {
    ...
    expect(MOCK_COLUMNS[1].getCellValue(result.current.getCellContent([1, 0]))).toEqual("bar");
  });
});
//# sourceMappingURL=useDataLoader.test.js.map