...
const CSV_SPECIAL_CHARS_REGEX = new RegExp("[".concat([CSV_DELIMITER, CSV_QUOTE_CHAR, CSV_ROW_DELIMITER].join(""), "]"));
...
function escapeValue(value) {
  ...
  if (CSV_SPECIAL_CHARS_REGEX.test(strValue)) {
    ...
    return "".concat(CSV_QUOTE_CHAR).concat(strValue.replace(
    // Escape all quote chars if inside a quoted string:
    new RegExp(CSV_QUOTE_CHAR, "g"), CSV_ESCAPE_CHAR + CSV_QUOTE_CHAR)).concat(CSV_QUOTE_CHAR);
  }
  return strValue;
}
/**
...
function useDataExporter(getCellContent, columns, numRows, enforceDownloadInNewTab) {
  const exportToCsv = React.useCallback(async () => {
    ...
    const suggestedName = "".concat(timestamp, "_export.csv");
    try {
