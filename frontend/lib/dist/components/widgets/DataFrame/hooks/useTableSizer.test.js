...
import useTableSizer, { calculateMaxHeight, MIN_TABLE_WIDTH } from "./useTableSizer";
describe("useTableSizer hook", () => {
  it("applies the configured width", () => {
    ...
    const {
      result
    } = renderHook(() => useTableSizer(ArrowProto.create({
      data: TEN_BY_TEN,
  ...
  it("applies the configured height", () => {
    ...
    expect(result.current.maxHeight).toEqual(calculateMaxHeight(NUMBER_OF_ROWS + 1));
  });
  ...
  it("calculates correct container dimensions for fullscreen mode", () => {
    ...
    // VERY_TALL table has 100 rows
    CONTAINER_WIDTH, CONTAINER_HEIGHT, true));
    expect(result.current.maxWidth).toEqual(CONTAINER_WIDTH);
  ...
  it("adapts size on resizing", () => {
    ...
    const {
      ...
      useContainerWidth: false,
      width: TABLE_WIDTH
    }), NUMBER_OF_ROWS, CONTAINER_WIDTH));
    const NEW_WIDTH = 100;
...
//# sourceMappingURL=useTableSizer.test.js.map