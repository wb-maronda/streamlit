...
import { getColumnTypeFromArrow, getAllColumnsFromArrow, getEmptyIndexColumn } from "../arrowUtils";
import { ObjectColumn, ColumnTypes } from "../columns";
...
export function applyColumnConfig(columnProps, columnConfigMapping) {
  ...
  } else if (columnConfigMapping.has("".concat(COLUMN_POSITION_PREFIX).concat(columnProps.indexNumber))) {
    // Config is configured based on the column position, e.g. col:0 -> first column
    columnConfig = columnConfigMapping.get("".concat(COLUMN_POSITION_PREFIX).concat(columnProps.indexNumber));
...
export function getColumnType(column) {
  var _column$columnTypeOpt;
  const customType = (_column$columnTypeOpt = column.columnTypeOptions) === null || _column$columnTypeOpt === void 0 ? void 0 : _column$columnTypeOpt.type;
  ...
  if (notNullOrUndefined(customType)) {
    if (ColumnTypes.has(customType)) {
      ...
      logWarning("Unknown column type configured in column configuration: ".concat(customType));
    }
  }
