...
const getOriginalIndexMock = jest.fn().mockImplementation(index => {
  return index;
});
const getCellContentMock = jest.fn().mockImplementation(_ref => {
  let [col] = _ref;
...
describe("useDataEditor hook", () => {
  ...
  it("allows to edit cells with onCellEdited", () => {
    ...
    const {
      ...
      return useDataEditor(MOCK_COLUMNS, false, editingState, getCellContentMock, getOriginalIndexMock, refreshCellsMock, updateNumRows, syncEditsMock, clearSelectionMock);
    });
    ...
    result.current.onCellEdited([1, 0], columnToEdit.getCell("bar"));
    ...
    expect(editingState.current.toJson(MOCK_COLUMNS)).toEqual('{"edited_rows":{"0":{"column_2":"bar"}},"added_rows":[],"deleted_rows":[]}');
  });
  it("correctly handles indices on editing", () => {
    ...
    const {
      ...
      return useDataEditor([{
        ...MOCK_COLUMNS[0],
        isIndex: true
      }, MOCK_COLUMNS[1]], false, editingState, getCellContentMock, getOriginalIndexMock, refreshCellsMock, updateNumRows, syncEditsMock, clearSelectionMock);
    });
    if (typeof result.current.onCellEdited !== "function") {
      throw new Error("onCellEdited is expected to be a function");
    }
  ...
  it("applies cell edits from pasted data via onPaste", () => {
    ...
    // Check with full editing state
    expect(editingState.current.toJson(MOCK_COLUMNS)).toEqual('{"edited_rows":{"1":{"column_1":321,"column_2":"bar"}},"added_rows":[],"deleted_rows":[]}');
  });
  it("adds new rows from pasted data via onPaste", () => {
    ...
    const {
      ...
      return useDataEditor(MOCK_COLUMNS, false,
      // activates addition & deletion of rows
      editingState, getCellContentMock, getOriginalIndexMock, refreshCellsMock, updateNumRows, syncEditsMock, clearSelectionMock);
    });
    if (typeof result.current.onPaste !== "function") {
      throw new Error("onPaste is expected to be a function");
    }
    ...
    result.current.onPaste([0, INITIAL_NUM_ROWS - 1], [["321", "bar", "baz"], ["432", "lorem", "ipsum"]]);
    ...
    // Check with full editing state:
    expect(editingState.current.toJson(MOCK_COLUMNS)).toEqual('{"edited_rows":{"2":{"column_1":321,"column_2":"bar"}},"added_rows":[{"column_1":432,"column_2":"lorem"}],"deleted_rows":[]}');
  });
  it("doesn't add new rows from pasted data via onPaste if fixed num rows", () => {
    ...
    const {
      ...
      return useDataEditor(MOCK_COLUMNS, true,
      // deactivate the addition of new rows
      editingState, getCellContentMock, getOriginalIndexMock, refreshCellsMock, updateNumRows, syncEditsMock, clearSelectionMock);
    });
    if (typeof result.current.onPaste !== "function") {
      throw new Error("onPaste is expected to be a function");
    ...
    // Check with full editing state:
    expect(editingState.current.toJson(MOCK_COLUMNS)).toEqual('{"edited_rows":{"2":{"column_1":321,"column_2":"bar"}},"added_rows":[],"deleted_rows":[]}');
  });
  ...
  it("uses default values for new rows in onRowAppended", () => {
    ...
    // Check with full editing state:
    expect(editingState.current.toJson(MOCK_COLUMNS)).toEqual('{"edited_rows":{},"added_rows":[{"column_2":"foo"}],"deleted_rows":[]}');
  });
  it("doesn't allow to add new rows via onRowAppended if fix num rows", () => {
    ...
    const {
      ...
      return useDataEditor(MOCK_COLUMNS, true,
      // deactivates addition & deletion of rows
      editingState, getCellContentMock, getOriginalIndexMock, refreshCellsMock, updateNumRows, syncEditsMock, clearSelectionMock);
    });
    if (typeof result.current.onRowAppended !== "function") {
      throw new Error("onRowAppended is expected to be a function");
  ...
  it("allows to delete cell content via onDelete", () => {
    ...
    expect(syncEditsMock).toHaveBeenCalled();
    expect(refreshCellsMock).toHaveBeenCalledWith([{
      cell: [0, 0]
    }]);
    ...
    // Check with full editing state
    expect(editingState.current.toJson(MOCK_COLUMNS)).toEqual('{"edited_rows":{"0":{"column_1":null}},"added_rows":[],"deleted_rows":[]}');
  });
  it("allows to delete rows via onDelete", () => {
    ...
    // Check with full editing state
    expect(editingState.current.toJson(MOCK_COLUMNS)).toEqual("{\"edited_rows\":{},\"added_rows\":[],\"deleted_rows\":[1]}");
  });
  it("doesn't allow to delete rows via onDelete if fix num rows", () => {
    ...
    // Check with full editing state
    expect(editingState.current.toJson(MOCK_COLUMNS)).toEqual("{\"edited_rows\":{},\"added_rows\":[],\"deleted_rows\":[]}");
  });
  it("calls validateInput and returns false on invalid data.", () => {
    ...
    const invalidValue = columnToValidate.getCell("12345");
    const validationResult = result.current.validateCell([1, 0], invalidValue, columnToValidate.getCell(undefined));
  ...
  it("calls validateInput and corrects invalid input.", () => {
    ...
    const columnToValidate = MOCK_COLUMNS[1];
    const invalidValue = columnToValidate.getCell("abcdefghijk");
    const validationResult = result.current.validateCell([1, 0], invalidValue, columnToValidate.getCell(undefined));
    expect(validationResult.data).toEqual("abcdefghij");
  });
  it("calls validateInput and returns true on valid data.", () => {
    ...
    const columnToValidate = MOCK_COLUMNS[1];
    const validValue = columnToValidate.getCell("abcde");
    const validResult = result.current.validateCell([1, 0], validValue, columnToValidate.getCell(undefined));
    expect(validResult).toEqual(true);
  });
});
//# sourceMappingURL=useDataEditor.test.js.map