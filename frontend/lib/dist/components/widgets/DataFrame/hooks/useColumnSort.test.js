...
describe("useColumnSort hook", () => {
  it("should correctly sort numbers ascending and descending order", () => {
    const {
      result
    } = renderHook(() => useColumnSort(MOCK_PROPS.numRows, MOCK_PROPS.columns, MOCK_PROPS.getCellContent));
    ...
    act(() => {
      ...
      sortColumn === null || sortColumn === void 0 ? void 0 : sortColumn(MOCK_COLUMNS[SELECTED_COLUMN].indexNumber);
    });
    ...
    for (let i = 0; i < MOCK_PROPS.numRows; i++) {
      sortedDataAsc.push(result.current.getCellContent([SELECTED_COLUMN, i]).data);
    }
    const sortOperator = (a, b) => {
      if (a === undefined) {
        return -1;
      }
      if (b === undefined) {
    ...
    for (let i = 0; i < MOCK_PROPS.numRows; i++) {
      sortedDataDesc.push(result.current.getCellContent([SELECTED_COLUMN, i]).data);
    }
...
//# sourceMappingURL=useColumnSort.test.js.map