 ...
 */
function useSelectionHandler(element, isEmptyTable, isDisabled, columns, syncSelectionState) {
  const [gridSelection, setGridSelection] = React.useState({
    columns: CompactSelection.empty(),
    rows: CompactSelection.empty(),
    current: undefined
  });
  const isRowSelectionActivated = !isEmptyTable && !isDisabled && (element.selectionMode.includes(ArrowProto.SelectionMode.MULTI_ROW) || element.selectionMode.includes(ArrowProto.SelectionMode.SINGLE_ROW));
  const isMultiRowSelectionActivated = isRowSelectionActivated && element.selectionMode.includes(ArrowProto.SelectionMode.MULTI_ROW);
  const isColumnSelectionActivated = !isEmptyTable && !isDisabled && (element.selectionMode.includes(ArrowProto.SelectionMode.SINGLE_COLUMN) || element.selectionMode.includes(ArrowProto.SelectionMode.MULTI_COLUMN));
  const isMultiColumnSelectionActivated = isColumnSelectionActivated && element.selectionMode.includes(ArrowProto.SelectionMode.MULTI_COLUMN);
   ...
   */
  const processSelectionChange = React.useCallback(newSelection => {
    const rowSelectionChanged = !isEqual(newSelection.rows.toArray(), gridSelection.rows.toArray());
    const columnSelectionChanged = !isEqual(newSelection.columns.toArray(), gridSelection.columns.toArray());
    const cellSelectionChanged = !isEqual(newSelection.current, gridSelection.current);

    // A flag to determine if the selection should be synced with the widget state
    let syncSelection = isRowSelectionActivated && rowSelectionChanged || isColumnSelectionActivated && columnSelectionChanged;
    let updatedSelection = newSelection;
    if ((isRowSelectionActivated || isColumnSelectionActivated) && newSelection.current !== undefined && cellSelectionChanged) {
    ...
    if (rowSelectionChanged && newSelection.rows.length > 0 && columnSelectionChanged && newSelection.columns.length === 0) {
    ...
    if (columnSelectionChanged && newSelection.columns.length > 0 && rowSelectionChanged && newSelection.rows.length === 0) {
  ...
  }, [gridSelection, isRowSelectionActivated, isColumnSelectionActivated, syncSelectionState, columns]);
  ...
  const clearSelection = React.useCallback(function () {
    let keepRows = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
    let keepColumns = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    const emptySelection = {
      columns: keepColumns ? gridSelection.columns : CompactSelection.empty(),
      rows: keepRows ? gridSelection.rows : CompactSelection.empty(),
      current: undefined
    };
    setGridSelection(emptySelection);
    if (!keepRows && isRowSelectionActivated || !keepColumns && isColumnSelectionActivated) {
      syncSelectionState(emptySelection);
    }
  }, [gridSelection, isRowSelectionActivated, isColumnSelectionActivated, syncSelectionState]);
  return {
    gridSelection,
