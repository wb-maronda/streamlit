...
import { SparklineCell, DropdownCell, RangeCell, DatePickerCell } from "@glideapps/glide-data-grid-cells";
import { CustomCells, isMissingValueCell } from "../columns";
 ...
 */
export function drawRequiredIndicator(ctx, rect, theme) {
  ctx.save();
  ctx.beginPath();
 ...
 */
export const drawMissingPlaceholder = args => {
  const {
    cell,
    theme,
    ctx
  ...
  drawTextCell({
    ...
    theme: {
      ...
      headerFontFull: "".concat(theme.headerFontStyle, " ").concat(theme.fontFamily),
      baseFontFull: "".concat(theme.baseFontStyle, " ").concat(theme.fontFamily),
      markerFontFull: "".concat(theme.markerFontStyle, " ").concat(theme.fontFamily)
    },
 ...
 */
function useCustomRenderer(columns) {
  const drawCell = React.useCallback((args, draw) => {
    ...
    if (isMissingValueCell(cell) && colPos < columns.length) {
      ...
      if (["checkbox", "line_chart", "bar_chart", "progress"].includes(column.kind)) {
        draw();
      } else {
  ...
  const customRenderers = React.useMemo(() => [SparklineCell, DropdownCell, RangeCell, DatePickerCell, ...CustomCells],
...
//# sourceMappingURL=useCustomRenderer.js.map