  ...
  const getCellContent = React.useCallback(_ref => {
    ...
    if (column.isEditable || isAddedRow) {
      ...
      if (notNullOrUndefined(editedCell)) {
        ...
        return getErrorCell("Error during cell creation.", "This should never happen. Please report this bug. " + "No cell found for an added row: col=".concat(originalCol, "; row=").concat(originalRow));
      }
    }
    try {
