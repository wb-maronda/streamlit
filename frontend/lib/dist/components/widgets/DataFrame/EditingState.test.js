...
const MOCK_TEXT_CELL_1 = {
  kind: GridCellKind.Text,
  displayData: "foo",
  data: "foo",
  allowOverlay: true
};
const MOCK_TEXT_CELL_2 = {
  kind: GridCellKind.Text,
  displayData: "foo",
  data: "foo",
  allowOverlay: true
...
describe("EditingState class", () => {
  ...
  it("allows to add and delete multiple rows", () => {
    ...
    let LAST_ROW_ID = editingState.getOriginalRowIndex(editingState.getNumRows() - 1);
    ...
    // Last row should be an edited row
    LAST_ROW_ID = editingState.getOriginalRowIndex(editingState.getNumRows() - 1);
    expect(editingState.isAddedRow(LAST_ROW_ID)).toEqual(true);
    expect(editingState.getCell(0, LAST_ROW_ID)).toEqual(MOCK_TEXT_CELL_1);
    expect(editingState.getCell(1, LAST_ROW_ID)).toEqual(MOCK_TEXT_CELL_2);

    // Delete remaining rows via deleteRows
    editingState.deleteRows([editingState.getOriginalRowIndex(0), editingState.getOriginalRowIndex(1), editingState.getOriginalRowIndex(2)]);
  ...
  it("ignores rows with required empty values in toJson", () => {
    ...
    // Convert to JSON
    const json = editingState.toJson([TextColumn({
      ...baseColumnProps,
      indexNumber: 0,
      id: "column_1"
    }), TextColumn({
    ...
    expect(json).toEqual('{"edited_rows":{},"added_rows":[],"deleted_rows":[]}');
  });
  it("converts editing state to JSON", () => {
    ...
    expect(json).toEqual('{"edited_rows":{"0":{"column_1":"foo"}},"added_rows":[{"column_1":"foo","column_2":"foo"}],"deleted_rows":[1]}');
  });
  it.each([["{\"edited_rows\":{\"0\":{\"column_1\":\"foo\"}},\"added_rows\":[{\"column_1\":\"foo\",\"column_2\":\"foo\"}],\"deleted_rows\":[1]}"], ["{\"edited_rows\":{},\"added_rows\":[],\"deleted_rows\":[]}"], ["{\"edited_rows\":{},\"added_rows\":[{\"column_1\":\"foo\",\"column_2\":\"foo\"}],\"deleted_rows\":[]}"], ["{\"edited_rows\":{},\"added_rows\":[],\"deleted_rows\":[1]}"], ["{\"edited_rows\":{\"0\":{\"column_1\":\"foo\"}},\"added_rows\":[],\"deleted_rows\":[]}"], ["{\"edited_rows\":{\"0\":{\"_index\":\"foo\"}},\"added_rows\":[],\"deleted_rows\":[]}"]])("converts JSON to editing state: %p", editingStateJson => {
    const NUM_OF_ROWS = 3;
    const editingState = new EditingState(NUM_OF_ROWS);
  ...
  it("ensure all cells of added rows are filled even if empty", () => {
    ...
    editingState.fromJson("{\"edited_rows\":{},\"added_rows\":[{\"column_1\":\"foo\"}],\"deleted_rows\":[]}", MOCK_COLUMNS);
...
//# sourceMappingURL=EditingState.test.js.map