...
import { AREA_CHART_TYPE, AreaChartColumn, BAR_CHART_TYPE, BarChartColumn, LINE_CHART_TYPE, LineChartColumn } from "./ChartColumn";
...
describe("ChartColumn", () => {
  it("creates a valid column instance", () => {
    ...
    expect(mockCell.data?.values).toEqual([0.1, 0.2, 0.3]);
    expect(mockCell.data?.displayValues).toEqual(["0.1", "0.2", "0.3"]);
  });
  it("supports configuring the chart type", () => {
    ...
    expect(mockCell.data?.graphKind).toEqual("line");
    ...
    expect(mockBarChartCell.data?.graphKind).toEqual("bar");
    ...
    expect(mockAreaChartCell.data?.graphKind).toEqual("area");
  });
  it("supports configuring min/max scale", () => {
    ...
    expect(mockCell.data?.values).toEqual([0, 0.5, 1]);
    ...
    expect(mockCell1.data?.values).toEqual([-100, 0, 100]);
    ...
    expect(mockCell2.data?.values).toEqual([-1, 0, 1]);
    ...
    // This should automatically normalize the values to the min/max scale:
    expect(mockCell3.data?.values).toEqual([0, 100, 200]);
    ...
    expect(mockCell4.data?.values).toEqual([-100, 0, 100]);
