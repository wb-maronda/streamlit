...
describe("LinkColumn", () => {
  ...
  it.each([["foo", "foo"], ["https://streamlit.io", "https://streamlit.io"], ["/path/to/file", "/path/to/file"], [null, null], [undefined, null]
  // All the values that are supported by the TextColumn
  // should also be supported by the UrlColumn.
  ])("supports string-compatible value (%p parsed as %p)", (input, value) => {
    const mockColumn = LinkColumn(MOCK_LINK_COLUMN_PROPS);
  ...
  it("validates input based on validate regex", () => {
    const mockColumn = LinkColumn({
      ...MOCK_LINK_COLUMN_PROPS,
      columnTypeOptions: {
        validate: "^https://(?:www.)?(?:[a-zA-Z0-9-]+.)*streamlit.app(?:/.*)?$"
      }
    });
    expect(mockColumn.validateInput("https://issues.streamlit.app/")).toBe(true);
    expect(mockColumn.validateInput("https://issues.streamlit.app/Streamlit_Issues_Leaderboard?issue=10")).toBe(true);
    expect(mockColumn.validateInput("issues.streamlit.app/")).toBe(false);
    expect(mockColumn.validateInput("https://issues.streamlit.io/")).toBe(false);
  });
  it("applies input validation in the getCell call based on max_chars and validate regex", () => {
    ...
    expect(isErrorCell(mockColumn.getCell("https://issues.streamlit.app/", true))).toBe(false);
    expect(isErrorCell(mockColumn.getCell("https://issues.streamlit.io/", true))).toBe(true);

    // We do not auto fix a link cell that's too long
    expect(isErrorCell(mockColumn.getCell("https://issues.streamlit.app/Streamlit_Issues_Leaderboard?issue=10", true))).toBe(true);
  });
  ...
  it("sets displayed value as the applied regex to the href when displayText is a regex", () => {
    ...
    const cell = mockColumn.getCell("https://roadmap.streamlit.app", true);
  ...
  it("sets displayed value as the applied regex to the href when displayText is a regex with URL encoding", () => {
    ...
    const cell = mockColumn.getCell("https://streamlit.app?app=foo%20app%20%25", true);
...
//# sourceMappingURL=LinkColumn.test.js.map