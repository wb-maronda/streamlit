...
const MOCK_DATETIME_COLUMN_TEMPLATE = {
  id: "1",
...
const MOCK_DATE_COLUMN_TEMPLATE = {
  id: "1",
  name: "date_column",
  title: "date_column",
  indexNumber: 0,
...
const MOCK_TIME_COLUMN_TEMPLATE = {
  id: "1",
  name: "time_column",
  title: "time_column",
  indexNumber: 0,
...
describe("DateTimeColumn", () => {
  it("creates a valid column instance", () => {
    ...
    expect(mockCell.data.date).toEqual(EXAMPLE_DATE);
  });
  it.each([
  // valid date object
  ...
  ["1671951600", "2022-12-25T07:00:00.000"]])("supports datetime-compatible value (%p parsed as %p)", (input, value) => {
    const mockColumn = DateTimeColumn(MOCK_DATETIME_COLUMN_TEMPLATE);
    const cell = mockColumn.getCell(input);
    expect(mockColumn.getCellValue(cell)).toEqual(value);
  });
  it.each([[NaN], ["foo"]])("%p results in error cell", input => {
    const mockColumn = DateTimeColumn(MOCK_DATETIME_COLUMN_TEMPLATE);
    const cell = mockColumn.getCell(input);
    expect(isErrorCell(cell)).toEqual(true);
  });
  it("respects min_value config option", () => {
    const MOCK_DATETIME_COLUMN_WITH_MIN = {
    ...
    expect(mockColumn.getCellValue(aboveMinValueCell)).toEqual("2023-04-25T23:59:59.000");
  ...
  it("respects max_value config option", () => {
    const MOCK_DATETIME_COLUMN_WITH_MAX = {
      ...MOCK_DATETIME_COLUMN_TEMPLATE,
      columnTypeOptions: {
    ...
    expect(mockColumn.getCellValue(belowMaxValueCell)).toEqual("2023-04-23T23:59:59.000");
  ...
  it("changes the step size based on the config option", () => {
    const MOCK_DATETIME_COLUMN_WITH_STEP = {
      ...MOCK_DATETIME_COLUMN_TEMPLATE,
      columnTypeOptions: {
        step: 60
      }
    ...
    expect(newCell.data.step).toBe("60");
  });
  it("formats according to the provided format config option", () => {
    ...
    expect(cell.data.displayDate).toEqual("Apr 25th, 2023 - 10:30");
  });
  it("supports columns with timezone", () => {
    const MOCK_DATETIME_COLUMN_TEMPLATE_WITH_TIMEZONE = {
      ...MOCK_DATETIME_COLUMN_TEMPLATE,
      arrowType: {
    ...
    const mockColumn = DateTimeColumn(MOCK_DATETIME_COLUMN_TEMPLATE_WITH_TIMEZONE);
    const mockCell = mockColumn.getCell(EXAMPLE_DATE);
    expect(mockCell.data.date).toEqual(EXAMPLE_DATE);
    expect(mockCell.data.displayDate).toEqual("2023-04-25 15:30:00+05:00");
    expect(mockCell.data.timezoneOffset).toEqual(300);
  });
});
describe("DateColumn", () => {
  it("creates a valid column instance", () => {
    ...
    const mockCell = mockColumn.getCell(EXAMPLE_DATE);
    expect(mockCell.kind).toEqual(GridCellKind.Custom);
    expect(mockCell.copyData).toEqual("2023-04-25");
  });
  it.each([
  // valid date object
  ...
  ["1671951600", "2022-12-25"]])("supports date-compatible value (%p parsed as %p)", (input, value) => {
    const mockColumn = DateColumn(MOCK_DATE_COLUMN_TEMPLATE);
    const cell = mockColumn.getCell(input);
    expect(mockColumn.getCellValue(cell)).toEqual(value);
  });
  ...
  it("respects min_value config option", () => {
    const MOCK_DATE_COLUMN_TEMPLATE_WITH_MIN = {
  ...
  it("respects max_value config option", () => {
    const MOCK_DATE_COLUMN_TEMPLATE_WITH_MAX = {
      ...MOCK_DATE_COLUMN_TEMPLATE,
      columnTypeOptions: {
  ...
  it("changes the step size based on the config option", () => {
    const MOCK_DATE_COLUMN_WITH_STEP = {
      ...MOCK_DATE_COLUMN_TEMPLATE,
      columnTypeOptions: {
        step: 2
      }
    ...
    const newCell = mockColumn.getCell(EXAMPLE_DATE);
    expect(newCell.data.step).toBe("2");
  });
  it("formats according to the provided format config option", () => {
    ...
    const cell = mockColumn.getCell(EXAMPLE_DATE);
    expect(cell.data.displayDate).toEqual("Apr 25th, 2023");
  });
});
describe("TimeColumn", () => {
  it("creates a valid column instance", () => {
    ...
    const mockCell = mockColumn.getCell(EXAMPLE_DATE);
    expect(mockCell.kind).toEqual(GridCellKind.Custom);
    expect(mockCell.copyData).toEqual("10:30:00");
  });
  it.each([
  // valid date object
  ...
  // valid Unix timestamp in milliseconds as a string
  ["1671951600", "07:00:00.000"]])("supports time-compatible value (%p parsed as %p)", (input, value) => {
    const mockColumn = TimeColumn(MOCK_TIME_COLUMN_TEMPLATE);
    const cell = mockColumn.getCell(input);
    expect(mockColumn.getCellValue(cell)).toEqual(value);
  });
  ...
  it("respects min_value config option", () => {
    const MOCK_TIME_COLUMN_TEMPLATE_WITH_MIN = {
  ...
  it("respects max_value config option", () => {
    const MOCK_TIME_COLUMN_TEMPLATE_WITH_MAX = {
      ...MOCK_TIME_COLUMN_TEMPLATE,
      columnTypeOptions: {
        max_value: "10:59:59"
      }
  ...
  it("changes the step size based on the config option", () => {
    const MOCK_TIME_COLUMN_WITH_STEP = {
      ...MOCK_TIME_COLUMN_TEMPLATE,
      columnTypeOptions: {
        step: 60
      }
  ...
  it("formats according to the provided format config option", () => {
    ...
    const cell = mockColumn.getCell(EXAMPLE_DATE);
    expect(cell.data.displayDate).toEqual("10:30");
  });
  it("adapts default format based on step size", () => {
    ...
    const mockColumn = TimeColumn(MOCK_TIME_COLUMN_WITH_STEP);
    const newCell = mockColumn.getCell(EXAMPLE_DATE);
    expect(newCell.data.displayDate).toBe("10:30");
  });
});
//# sourceMappingURL=DateTimeColumn.test.js.map