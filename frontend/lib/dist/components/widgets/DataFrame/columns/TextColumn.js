...
import { getErrorCell, toSafeString, removeLineBreaks } from "./utils";
/**
...
function TextColumn(props) {
  ...
  const validateInput = data => {
    ...
    if (validateRegex instanceof RegExp && validateRegex.test(cellData) === false) {
      return false;
    }
  ...
  return {
    ...
    getCell(data, validate) {
      ...
      try {
        ...
        const displayData = notNullOrUndefined(cellData) ? removeLineBreaks(cellData) // Remove line breaks to show all content in the cell
        : "";
        return {
          ...cellTemplate,
        ...
        return getErrorCell("Incompatible value", "The value cannot be interpreted as string. Error: ".concat(error));
      }
    },
    getCellValue(cell) {
