...
describe("CheckboxColumn", () => {
  it("creates a valid column instance", () => {
    ...
    expect(mockCell.data).toEqual(true);
  });
  it.each([[true, true], [false, false], ["true", true], ["false", false], ["yes", true], ["no", false], ["t", true], ["f", false], ["y", true], ["n", false], ["on", true], ["off", false], ["1", true], ["0", false], [1, true], [0, false], [[], null], [null, null], [undefined, null], ["", null]])("supports boolean compatible value (%p parsed as %p)", (input, value) => {
    const mockColumn = CheckboxColumn(MOCK_CHECKBOX_COLUMN_PROPS);
    const cell = mockColumn.getCell(input);
    expect(mockColumn.getCellValue(cell)).toEqual(value);
    expect(isErrorCell(cell)).toEqual(false);
  });
  it.each([["foo"], [12345], [0.1], [["foo", "bar"]]])("%p results in error cell: %p", input => {
    const mockColumn = CheckboxColumn(MOCK_CHECKBOX_COLUMN_PROPS);
    const cell = mockColumn.getCell(input);
    expect(isErrorCell(cell)).toEqual(true);
  });
});
//# sourceMappingURL=CheckboxColumn.test.js.map