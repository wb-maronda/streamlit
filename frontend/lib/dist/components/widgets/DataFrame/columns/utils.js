...
export function formatNumber(value, format, maxPrecision) {
  ...
  if (isNullOrUndefined(format) || format === "") {
    ...
    return numbro(value).format(notNullOrUndefined(maxPrecision) ? `0,0.${"0".repeat(maxPrecision)}` : `0,0.[0000]` // If no precision is given, use 4 decimals and hide trailing zeros
    );
  }
  if (format === "percent") {
