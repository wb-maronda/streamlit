 ...
 */
export function hasTooltip(cell) {
  return cell.hasOwnProperty("tooltip") && cell.tooltip !== "";
}
/**
 ...
 */
export function isMissingValueCell(cell) {
...
export function getEmptyCell() {
  let missingCell = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  if (missingCell) {
    return {
 ...
 */
export function getTextCell(readonly, faded) {
 ...
 */
export function toGlideColumn(column) {
  return {
    id: column.id,
    title: column.title,
    hasMenu: false,
 ...
 */
export function mergeColumnParameters(defaultParams, userParams) {
 ...
 */
export function toSafeString(data) {
  try {
    try {
 ...
 */
export function formatNumber(value, format, maxPrecision) {
  ...
  if (isNullOrUndefined(format) || format === "") {
    ...
    return numbro(value).format(notNullOrUndefined(maxPrecision) ? "0,0.".concat("0".repeat(maxPrecision)) : "0,0.[0000]" // If no precision is given, use 4 decimals and hide trailing zeros
    );
  }
  if (format === "percent") {
    ...
    return Quiver.formatPeriodType(BigInt(value), format);
  }
 ...
 */
export function formatMoment(momentDate, format) {
  if (format === "locale") {
...
export function toSafeDate(value) {
  ...
  try {
    ...
    if (typeof value === "string") {
      ...
      const parsedMomentTime = moment.utc(value, [moment.HTML5_FMT.TIME_MS,
      // HH:mm:ss.SSS
...
export function truncateDecimals(value, decimals) {
  return decimals === 0 ? Math.trunc(value) : Math.trunc(value * 10 ** decimals) / 10 ** decimals;
}
 ...
 */
export function getLinkDisplayValueFromRegex(displayTextRegex, href) {
