...
const MOCK_CATEGORICAL_TYPE = {
  pandas_type: "int8",
  numpy_type: "categorical"
};
const MOCK_BOOLEAN_ARROW_TYPE = {
  pandas_type: "bool",
  numpy_type: "bool"
};
const SELECTBOX_COLUMN_TEMPLATE = {
  id: "1",
...
function getSelectboxColumn(arrowType, params, column_props_overwrites) {
...
describe("SelectboxColumn", () => {
  it("creates a valid column instance with string values", () => {
    ...
    expect(mockCell.data.allowedValues).toEqual([null, "foo", "bar"]);
  });
  it("creates a valid column instance number values", () => {
    ...
    expect(mockCell.data.allowedValues).toEqual([null, "1", "2", "3"]);
  });
  it("creates a valid column instance from boolean type", () => {
    ...
    expect(mockCell.data.allowedValues).toEqual([null, "true", "false"]);
  });
  it("creates a required column that does not add the empty value", () => {
    ...
    const mockCell = mockColumn.getCell("foo");
    expect(mockCell.data.allowedValues).toEqual(["foo", "bar"]);
  ...
  it.each([[null], [undefined], [""]])("%p is interpreted as missing value", input => {
    const mockColumn = getSelectboxColumn(MOCK_CATEGORICAL_TYPE, {
      options: ["foo", "bar"]
    });
...
//# sourceMappingURL=SelectboxColumn.test.js.map