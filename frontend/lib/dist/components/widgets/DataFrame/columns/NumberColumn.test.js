...
const MOCK_FLOAT_ARROW_TYPE = {
  pandas_type: "float64",
  numpy_type: "float64"
};
const MOCK_INT_ARROW_TYPE = {
  pandas_type: "int64",
  numpy_type: "int64"
};
const MOCK_UINT_ARROW_TYPE = {
  pandas_type: "uint64",
  numpy_type: "uint64"
};
...
function getNumberColumn(arrowType, params) {
  return NumberColumn({
...
describe("NumberColumn", () => {
  it("creates a valid column instance", () => {
    ...
    expect(mockCell.displayData).toEqual("1.234");
    expect(mockCell.data).toEqual(1.234);
  });
  ...
  it.each([[true, 1], [false, 0], ["4.12", 4.12], ["-4.12", -4.12], ["4", 4], [1.3122, 1.3122], [-1.3122, -1.3122], ["1,212.12", 1212.12], [".1312314", 0.1312314], [null, null], [undefined, null], ["", null]])("supports float64 value (%p parsed as %p)", (input, value) => {
    const mockColumn = getNumberColumn(MOCK_FLOAT_ARROW_TYPE);
    const cell = mockColumn.getCell(input);
    expect(mockColumn.getCellValue(cell)).toEqual(value);
  });
  it.each([[100, 100], [-100, -100], ["4", 4], ["4.12", 4], ["4.61", 4], ["-4.12", -4], [1.3122, 1], [-1.3122, -1], ["1,212", 1212], ["1,212,123,312", 1212123312], [null, null]])("supports integer value (%p parsed as %p)", (input, value) => {
  ...
  it("properly configures the column for unsigned integers", () => {
    ...
    expect(mockCell.kind).toEqual(GridCellKind.Number);
    expect(mockCell.fixedDecimals).toEqual(0);
    expect(mockCell.allowNegative).toEqual(false);
  });
  it.each([[100, true], [-100, false], ["4", true], ["-4.12", false]])("supports unsigned integer validation (%p validates to %p)", (input, valid) => {
    const mockColumn = getNumberColumn(MOCK_UINT_ARROW_TYPE);
    expect(mockColumn.validateInput(input)).toEqual(valid);
  });
  it.each([[0, 1.234567, 1], [0.1, 1.234567, 1.2], [0.01, 1.234567, 1.23], [0.001, 1.234567, 1.234], [0.0001, 1.234567, 1.2345], [0.001, 1.1, 1.1], [0.00000001, 1, 1]])("converts value to precision from step %p (%p converted to %p)", (step, input, value) => {
    const mockColumn = getNumberColumn(MOCK_FLOAT_ARROW_TYPE, {
      step
    });
  ...
  it.each([[0, 1.234567, "1"], [0.1, 1.234567, "1.2"], [0.01, 1.234567, "1.23"], [0.001, 1.234567, "1.234"], [0.0001, 1.234567, "1.2345"], [0.001, 1.1, "1.100"], [0.00000001, 1, "1.00000000"]])("correctly adapts default value to precision from step %p (%p displayed as %p)", (step, input, displayValue) => {
    const mockColumn = getNumberColumn(MOCK_FLOAT_ARROW_TYPE, {
      step
    });
    const mockCell = mockColumn.getCell(input);
    expect(mockCell.displayData).toEqual(displayValue);
  });
  it.each([[10, 10, true], [10, 100, true], [10, 5, false], [10, -5, false]])("supports minimal value configuration %p (%p validates to %p)", (min_value, input, valid) => {
    const mockColumn = getNumberColumn(MOCK_FLOAT_ARROW_TYPE, {
      min_value
    });
    expect(mockColumn.validateInput(input)).toEqual(valid);
  });
  it.each([[10, 10, true], [10, 100, 10], [10, 5, true], [10, -5, true]])("supports maximal value configuration %p (%p validates to %p)", (max_value, input, validation) => {
    const mockColumn = getNumberColumn(MOCK_FLOAT_ARROW_TYPE, {
      max_value
    });
    expect(mockColumn.validateInput(input)).toEqual(validation);
  });
  it.each([[[]], ["foo"], [[1, 2]], ["123.124.123"], ["--123"], ["2,,2"], ["12345678987654321"]])("%p results in error cell", input => {
    const mockColumn = getNumberColumn(MOCK_FLOAT_ARROW_TYPE);
    const cell = mockColumn.getCell(input);
  ...
  [10.123, "%d", "10"], [10.123, "%i", "10"], [10.123, "%u", "10"], [10.123, "%f", "10.123"], [10.123, "%g", "10.123"], [10, "$%.2f", "$10.00"], [10.126, "$%.2f", "$10.13"], [10.123, "%.2f€", "10.12€"], [10.126, "($%.2f)", "($10.13)"], [65, "%d years", "65 years"], [1234567898765432, "%d ⭐", "1234567898765432 ⭐"], [72.3, "%.1f%%", "72.3%"], [-5.678, "%.1f", "-5.7"], [0.12, "percent", "12.00%"], [1100, "compact", "1.1K"]])("formats %p with sprintf format %p to %p", (input, format, displayValue) => {
    ...
    const cell = mockColumn.getCell(input);
    expect(cell.displayData).toEqual(displayValue);
  });
  ...
  it.each([[10, "10"], [1234567, "1234567"], [12345.678, "12345.678"], [-0.000123456, "-0.000123456"], [null, ""], [undefined, ""]])("uses raw number for copyData so that %p is copied as %p", (input, expectedCopyData) => {
    const mockColumn = getNumberColumn(MOCK_FLOAT_ARROW_TYPE, {
      format: "$%.2f"
    });
    const cell = mockColumn.getCell(input);
...
//# sourceMappingURL=NumberColumn.test.js.map