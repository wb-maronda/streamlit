...
import { LabelVisibilityMessage as LabelVisibilityMessageProto, NumberInput as NumberInputProto } from "../../../proto";
import { render } from "../../../test_util";
import { WidgetStateManager } from "../../../WidgetStateManager";
import { mockTheme } from "../../../mocks/mockTheme";
import { canDecrement, canIncrement, formatValue, NumberInput } from "./NumberInput";
...
describe("NumberInput widget", () => {
  ...
  it("removes the focused class when running onBlur", () => {
    ...
    expect(screen.getByTestId("stNumberInputContainer")).not.toHaveClass("focused");
  });
  ...
  it("shows Input Instructions on dirty state when not in form (by default)", async () => {
    ...
    expect(screen.getByTestId("InputInstructions")).toHaveTextContent("Press Enter to apply");
  });
  it("shows Input Instructions if in form that allows submit on enter", async () => {
    ...
    await user.click(numberInput);
    await user.keyboard("{backspace}5");
    expect(screen.getByTestId("InputInstructions")).toHaveTextContent("Press Enter to submit form");
  });
  it("hides Input Instructions if in form that doesn't allow submit on enter", async () => {
    ...
    jest.spyOn(props.widgetMgr, "allowFormEnterToSubmit").mockReturnValue(false);
    render( /*#__PURE__*/_jsx(NumberInput, {
      ...props
    }));
    const numberInput = screen.getByTestId("stNumberInputField");
  ...
  describe("FloatData", () => {
    ...
    describe("Formatting", () => {
      it("allows explicit formatting string", () => {
        ...
        expect(screen.getByTestId("stNumberInput")).toBeInTheDocument();
        expect(screen.getByTestId("stNumberInputField")).toHaveDisplayValue("1.1111");
      });
    });
    ...
    it("automatically sets formatting when none provided based on step", () => {
      ...
      expect(screen.getByTestId("stNumberInput")).toBeInTheDocument();
      expect(screen.getByTestId("stNumberInputField")).toHaveDisplayValue("1.000");
    });
    it("does not automatically format when a format is explicitly provided", () => {
      ...
      expect(screen.getByTestId("stNumberInput")).toBeInTheDocument();
      expect(screen.getByTestId("stNumberInputField")).toHaveDisplayValue("1.00");
    });
  ...
  describe("IntData", () => {
    ...
    it("can pass fragmentId to setIntValue", () => {
      ...
      expect(props.widgetMgr.setIntValue).toHaveBeenCalledWith(expect.anything(), 10, {
        fromUi: false
      }, "myFragmentId");
    });
    it("sets initialValue from widgetMgr", () => {
  ...
  describe("Step", () => {
    ...
    it("hides stepUp and stepDown buttons when width is smaller than 120px", () => {
      ...
      expect(screen.queryByTestId("stNumberInputStepUp")).not.toBeInTheDocument();
      expect(screen.queryByTestId("stNumberInputStepDown")).not.toBeInTheDocument();
    });
