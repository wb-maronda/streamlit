...
import { LabelVisibilityMessage as LabelVisibilityMessageProto, TextInput as TextInputProto } from "../../../proto";
...
describe("TextInput widget", () => {
  ...
  it("handles TextInputProto.autocomplete", () => {
    ...
    expect(autocompleteTextInput).toHaveAttribute("autoComplete", "one-time-password");
  });
  ...
  it("sets widget value on blur", () => {
    ...
    expect(props.widgetMgr.setStringValue).toHaveBeenCalledWith(props.element, "testing", {
      fromUi: true
    }, undefined);
  });
  it("sets widget value when enter is pressed", async () => {
    ...
    expect(props.widgetMgr.setStringValue).toHaveBeenLastCalledWith(props.element, "testing", {
      fromUi: true
    }, undefined);
  });
  ...
  it("does update widget value on text changes when inside of a form", async () => {
    ...
    expect(await screen.findByText("Press Enter to submit form")).toBeInTheDocument();
