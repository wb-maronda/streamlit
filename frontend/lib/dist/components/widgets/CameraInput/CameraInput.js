...
import { StyledBox, StyledCameraInput, StyledImg, StyledSpan } from "./styled-components";
...
class CameraInput extends React.PureComponent {
  constructor(props) {
    ...
    this.handleCapture = imgSrc => {
      ...
      return urltoFile(imgSrc, `camera-input-${new Date().toISOString().replace(/:/g, "_")}.jpg`).then(file => this.props.uploadClient.fetchFileURLs([file]).then(fileURLsArray => ({
        file: file,
