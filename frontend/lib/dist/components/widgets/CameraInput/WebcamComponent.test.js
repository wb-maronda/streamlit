...
describe("Test Webcam Component", () => {
  it("renders without crashing", () => {
    ...
    render( /*#__PURE__*/_jsx(WebcamComponent, {
      ...props
    }));
    expect(screen.getByTestId("stCameraInputWebcamComponent")).toBeInTheDocument();
  });
  it("renders ask permission screen when pending state", () => {
    ...
    expect(screen.getByTestId("stCameraInputWebcamComponent")).toBeInTheDocument();
    expect(screen.getByText("This app would like to use your camera.")).toBeInTheDocument();
    expect(screen.getByRole("link")).toHaveTextContent("Learn how to allow access.");
    // hidden style should be there and webcam should not show
    expect(screen.getByTestId("stCameraInputWebcamStyledBox")).toHaveAttribute("hidden");
  });
  it("renders ask permission screen when error state", () => {
    const props = getProps({
      testOverride: WebcamPermission.ERROR
    });
    render( /*#__PURE__*/_jsx(WebcamComponent, {
      ...props
    }));
  ...
  it("does not render ask permission screen in success state", () => {
    ...
    expect(screen.getByTestId("stCameraInputWebcamStyledBox")).not.toHaveAttribute("hidden");
  });
  ...
  it("changes `facingMode` when SwitchFacingMode button clicked", () => {
    ...
    expect(screen.getByTestId("stCameraInputSwitchButton")).toBeInTheDocument();
    const switchButton = within(screen.getByTestId("stCameraInputSwitchButton")).getByRole("button");
...
//# sourceMappingURL=WebcamComponent.test.js.map