...
import { FileUploader as FileUploaderProto, FileUploaderState as FileUploaderStateProto, FileURLs as FileURLsProto, LabelVisibilityMessage as LabelVisibilityMessageProto, UploadedFileInfo as UploadedFileInfoProto } from "../../../proto";
import { WidgetStateManager } from "../../../WidgetStateManager";
import FileUploader from "./FileUploader";
import { jsx as _jsx } from "react/jsx-runtime";
const createFile = function () {
  let filename = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "filename.txt";
  return new File(["Text in a file!"], filename, {
    type: "text/plain",
    lastModified: 0
  });
};
const buildFileUploaderStateProto = fileUrlsArray => new FileUploaderStateProto({
  uploadedFileInfo: fileUrlsArray.map(fileUrls => new UploadedFileInfoProto({
...
describe("FileUploader widget tests", () => {
  it("renders without crashing", () => {
    ...
    render( /*#__PURE__*/_jsx(FileUploader, {
      ...props
    }));
  ...
  it("sets initial value properly non-empty", () => {
    ...
    widgetMgr.setFileUploaderStateValue(element, buildFileUploaderStateProto([new FileURLsProto({
      fileId: "filename.txt",
  ...
  it("uploads a single file upload", async () => {
    ...
    expect((_fileDropZoneInput$fi = fileDropZoneInput.files) === null || _fileDropZoneInput$fi === void 0 ? void 0 : _fileDropZoneInput$fi[0]).toEqual(fileToUpload);
    expect(props.widgetMgr.setFileUploaderStateValue).toHaveBeenCalledWith(props.element, buildFileUploaderStateProto([{
      fileId: "filename.txt",
      uploadUrl: "filename.txt",
      deleteUrl: "filename.txt"
    }]), {
  ...
  it("uploads a single file even if too many files are selected", async () => {
    ...
    const filesToUpload = [new File(["Text in a file!"], "filename1.txt", {
      type: "text/plain",
      lastModified: 0
    ...
    await waitFor(() => expect(props.uploadClient.uploadFile).toHaveBeenCalledTimes(1));
  ...
  it("replaces file on single file uploader", async () => {
    var _fileDropZoneInput$fi2, _fileDropZoneInput$fi3;
    const user = userEvent.setup();
    const props = getProps();
    jest.spyOn(props.widgetMgr, "setFileUploaderStateValue");
    render( /*#__PURE__*/_jsx(FileUploader, {
    ...
    const fileName = screen.getByTestId("stFileUploaderFile");
    expect(fileName.textContent).toContain("filename.txt");
    expect((_fileDropZoneInput$fi2 = fileDropZoneInput.files) === null || _fileDropZoneInput$fi2 === void 0 ? void 0 : _fileDropZoneInput$fi2[0]).toEqual(firstFile);
    ...
    expect((_fileDropZoneInput$fi3 = fileDropZoneInput.files) === null || _fileDropZoneInput$fi3 === void 0 ? void 0 : _fileDropZoneInput$fi3[0]).toEqual(secondFile);
  ...
  it("uploads multiple files, even if some have errors", async () => {
    ...
    await waitFor(() => expect(props.uploadClient.uploadFile).toHaveBeenCalledTimes(2));
    const fileNames = screen.getAllByTestId("stFileUploaderFile");
    expect(fileNames.length).toBe(3);
    const errorFileNames = screen.getAllByTestId("stFileUploaderFileErrorMessage");
  ...
  it("can delete completed upload", async () => {
    ...
    expect(props.widgetMgr.setFileUploaderStateValue).toHaveBeenCalledTimes(3);
    expect(props.widgetMgr.setFileUploaderStateValue).toHaveBeenLastCalledWith(props.element, buildFileUploaderStateProto([{
      fileId: "filename1.txt",
      uploadUrl: "filename1.txt",
      deleteUrl: "filename1.txt"
    }, {
  ...
  it("can delete file with ErrorStatus", async () => {
    ...
    const fileDropZone = screen.getByTestId("stFileUploaderDropzone");
    const filesToUpload = [new File(["Another PDF file"], "anotherpdffile.pdf", {
      type: "application/pdf",
      lastModified: 0
    })];
    ...
    await waitFor(() => expect(screen.getAllByTestId("stFileUploaderFile").length).toBe(1));
    const errorFileNames = screen.getAllByTestId("stFileUploaderFileErrorMessage");
    expect(errorFileNames.length).toBe(1);
  ...
  it("handles upload error", async () => {
    ...
    props.uploadClient.uploadFile = jest.fn().mockRejectedValue(new Error("random upload error!"));
    await user.upload(fileDropZoneInput, createFile());
  ...
  it("shows an ErrorStatus when File extension is not allowed", async () => {
    ...
    const fileDropZone = screen.getByTestId("stFileUploaderDropzone");
    const filesToUpload = [new File(["TXT file"], "txtfile.txt", {
      type: "text/plain",
      lastModified: 0
    })];
    ...
    await waitFor(() => expect(screen.getAllByTestId("stFileUploaderFile").length).toBe(1));
    const errorFileNames = screen.getByTestId("stFileUploaderFileErrorMessage");
    expect(errorFileNames.textContent).toContain("text/plain files are not allowed.");
  });
  it("shows an ErrorStatus when maxUploadSizeMb = 0", async () => {
    ...
    await user.upload(fileDropZoneInput, createFile());
    const errorFileNames = screen.getByTestId("stFileUploaderFileErrorMessage");
    expect(errorFileNames.textContent).toContain("File must be 0.0B or smaller.");
  });
  it("resets its value when form is cleared", async () => {
    ...
    rerender( /*#__PURE__*/_jsx(FileUploader, {
      ...props
    }));
