...
import { ArrowDataframe, ComponentInstance as ComponentInstanceProto } from "../../../proto";
import { WidgetStateManager } from "../../../WidgetStateManager";
import { createIframeMessageHandler, CUSTOM_COMPONENT_API_VERSION, parseArgs, sendRenderMessage } from "./componentUtils";
...
describe("test componentUtils", () => {
  describe("createIframeMsgHandler", () => {
    ...
    let widgetMgr;
    let setComponentError;
    let componentReadyCallback;
    let frameHeightCallback;
    let ref;
    let iframeMessageHandler;
    beforeEach(() => {
  ...
  describe("sendRenderMessage", () => {
    it("should send message to iframe", () => {
      ...
      const disabled = true;
      sendRenderMessage(args, dataframeArgs, disabled, mockTheme.emotion, mockIframe);
  ...
  describe("parseArgs", () => {
    it("should parse jsonArgs and specialArgs", () => {
      ...
      const [newArgs, dataframeArgs] = parseArgs(JSON.stringify(args), specialArgs);
      expect(newArgs).toMatchObject({
        foo: "bar",
        "some-bytes": someBytes
      });
    ...
    it("should throw an error with with unknown specialArgs type", () => {
      ...
      expect(() => parseArgs(JSON.stringify(args), specialArgs)).toThrowError(Error);
    });
  });
});
//# sourceMappingURL=componentUtils.test.js.map