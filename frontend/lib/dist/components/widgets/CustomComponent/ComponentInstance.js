...
function ComponentInstance(props) {
  ...
  useEffect(() => {
    ...
    sendRenderMessage(parsedArgsRef.current.args, parsedArgsRef.current.dataframeArgs, disabled, theme, iframeRef.current ?? undefined);
  ...
  useEffect(() => {
    const contentWindow = iframeRef.current?.contentWindow ?? undefined;
    if (!contentWindow) {
      return;
    }
