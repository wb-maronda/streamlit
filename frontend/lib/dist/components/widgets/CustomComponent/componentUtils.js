  ...
  return (type, data) => {
    ...
    switch (type) {
      case ComponentMessageType.COMPONENT_READY:
        {
          ...
          if (apiVersion !== CUSTOM_COMPONENT_API_VERSION) {
            ...
            setComponentError(new Error("Unrecognized component API version: '".concat(apiVersion, "'")));
          } else {
            componentReadyCallback();
          }
          break;
      ...
      case ComponentMessageType.SET_COMPONENT_VALUE:
        if (!isReady) {
          logWarning("Got ".concat(type, " before ").concat(ComponentMessageType.COMPONENT_READY, "!"));
        } else {
      ...
      case ComponentMessageType.SET_FRAME_HEIGHT:
        ...
        break;
      default:
        logWarning("Unrecognized ComponentBackMsgType: ".concat(type));
    }
  };
}
...
export function parseArgs(jsonArgs, specialArgs) {
  ...
  for (const specialArg of specialArgs) {
    var _specialArg$value;
    const {
      key
    } = specialArg;
    switch ((_specialArg$value = specialArg.value) === null || _specialArg$value === void 0 ? void 0 : _specialArg$value.toLowerCase()) {
      case "arrowdataframe":
        dataframeArgs.push({
          key,
      ...
      case "bytes":
        ...
        break;
      default:
        throw new Error("Unrecognized SpecialArg type: ".concat(specialArg.value));
    }
  }
