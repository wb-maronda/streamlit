...
describe("ComponentInstance", () => {
  ...
  describe("SET_COMPONENT_VALUE handler", () => {
    ...
    it("warns if called before COMPONENT_READY", () => {
      ...
      expect(logWarning).toHaveBeenCalledWith(`Got ${ComponentMessageType.SET_COMPONENT_VALUE} before ${ComponentMessageType.COMPONENT_READY}!`);
    });
    describe("SET_FRAME_HEIGHT handler", () => {
      ...
      it("warns if called before COMPONENT_READY", () => {
        ...
        const widgetMgr = WidgetStateManager.mock.instances[0];
        expect(widgetMgr.setJsonValue).not.toHaveBeenCalled();
        expect(logWarning).toHaveBeenCalledWith(`Got ${ComponentMessageType.SET_FRAME_HEIGHT} before ${ComponentMessageType.COMPONENT_READY}!`);
      });
    });
  });
