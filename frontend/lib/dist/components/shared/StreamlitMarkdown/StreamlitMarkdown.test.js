...
import StreamlitMarkdown, { LinkWithTargetBlank, createAnchorFromText, CustomCodeTag } from "./StreamlitMarkdown";
import IsDialogContext from "../../core/IsDialogContext";

// Fixture Generator
import { jsx as _jsx } from "react/jsx-runtime";
const getMarkdownElement = body => {
  const components = {
...
describe("createAnchorFromText", () => {
  it("generates slugs correctly", () => {
    const cases = [["some header", "some-header"], ["some -24$35-9824  header", "some-24-35-9824-header"], ["blah___blah___blah", "blah-blah-blah"]];
    cases.forEach(_ref => {
      let [s, want] = _ref;
...
describe("linkReference", () => {
  it("renders a link with _blank target", () => {
    ...
    expect(screen.getByText("Streamlit")).toHaveAttribute("href", "https://streamlit.io/");
  ...
  it("renders a link containing a title", () => {
    const body = "My favorite search engine is " + '[Duck Duck Go](https://duckduckgo.com/ "The best search engine for privacy").';
    render(getMarkdownElement(body));
  ...
  it("renders a link containing parentheses", () => {
    const body = "Here's a link containing parentheses [Yikes](http://msdn.microsoft.com/en-us/library/aa752574(VS.85).aspx)";
    render(getMarkdownElement(body));
    const link = screen.getByText("Yikes");
    expect(link instanceof HTMLAnchorElement).toBe(true);
    expect(link).toHaveAttribute("href", "http://msdn.microsoft.com/en-us/library/aa752574(VS.85).aspx");
  });
  it("does not render a link if only [text] and no (href)", () => {
    ...
    expect(element).toHaveTextContent("Don't convert to a link if only [text] and missing (href)");
...
describe("StreamlitMarkdown", () => {
  it("renders header anchors when isInSidebar is false", () => {
    ...
    expect(screen.getByTestId("stHeadingWithActionElements")).toBeInTheDocument();
  });
  ...
  it("passes props properly", () => {
    const source = "<a class='nav_item' href='//0.0.0.0:8501/?p=some_page' target='_self'>Some Page</a>";
    render( /*#__PURE__*/_jsx(StreamlitMarkdown, {
      source: source,
      allowHTML: true
    }));
    expect(screen.getByText("Some Page")).toHaveAttribute("href", "//0.0.0.0:8501/?p=some_page");
  ...
  it("doesn't render header anchors when isInSidebar is true", () => {
    ...
    expect(screen.queryByTestId("stHeadingWithActionElements")).not.toBeInTheDocument();
  });
  ...
  const validCases = [{
    input: "*Italicized Text*",
    tag: "em",
    expected: "Italicized Text"
  }, {
    input: "**Bold Text**",
    tag: "strong",
    expected: "Bold Text"
    ...
    input: "[Link Text](www.example.com)",
    tag: "a",
    expected: "Link Text"
  }, {
    input: "🐶",
  ...
  test.each(validCases)("renders valid markdown when isLabel is true - $tag", _ref2 => {
    let {
      input,
      tag,
      expected
  ...
  test.each(invalidCases)("does NOT render invalid markdown when isLabel is true - $tag", _ref3 => {
    let {
      input,
      tag,
      expected
  ...
  it("doesn't render images when isLabel is true", () => {
    const source = "![Image Text](https://dictionary.cambridge.org/us/images/thumb/corgi_noun_002_08554.jpg?version=5.0.297)";
    render( /*#__PURE__*/_jsx(StreamlitMarkdown, {
      source: source,
      allowHTML: false,
      isLabel: true
  ...
  it("colours text properly", () => {
    const colorMapping = new Map([["red", colors.red80], ["blue", colors.blue80], ["green", colors.green90], ["violet", colors.purple80], ["orange", colors.orange100], ["gray", colors.gray80], ["grey", colors.gray80], ["rainbow", "transparent"]]);
    colorMapping.forEach(function (style, color) {
      const source = ":".concat(color, "[text]");
      render( /*#__PURE__*/_jsx(StreamlitMarkdown, {
        source: source,
        allowHTML: false
      }));
      ...
      expect(markdown).toHaveStyle("color: ".concat(style));
  ...
  it("properly adds background colors", () => {
    ...
    const colorMapping = new Map([["red", redbg], ["blue", bluebg], ["green", greenbg], ["violet", violetbg], ["orange", orangebg], ["gray", graybg], ["grey", graybg], ["rainbow", "linear-gradient(to right, ".concat(redbg, ", ").concat(orangebg, ", ").concat(yellowbg, ", ").concat(greenbg, ", ").concat(bluebg, ", ").concat(violetbg, ", ").concat(purplebg, ")")]]);
    colorMapping.forEach(function (style, color) {
      const source = ":".concat(color, "-background[text]");
      render( /*#__PURE__*/_jsx(StreamlitMarkdown, {
        source: source,
        allowHTML: false
      }));
      ...
      const tagName = markdown.nodeName.toLowerCase();
      expect(tagName).toBe("span");
      expect(markdown).toHaveStyle("background-color: ".concat(style));
...
const getCustomCodeTagProps = function () {
...
describe("CustomCodeTag Element", () => {
  it("should render without crashing", () => {
    ...
    render( /*#__PURE__*/_jsx(CustomCodeTag, {
      ...props
    }));
  ...
  it("should render inline", () => {
    ...
    expect(baseElement.innerHTML).toBe("<div><code>" + "import streamlit as st\n\n" + 'st.write("Hello")\n' + "</code></div>");
  });
});
//# sourceMappingURL=StreamlitMarkdown.test.js.map