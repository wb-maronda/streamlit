...
import BaseButton, { BaseButtonSize, BaseButtonKind } from "./BaseButton";
...
describe("Button element", () => {
  Object.keys(BaseButtonKind).forEach(key => {
    ...
    it("renders ".concat(kind, " buttons correctly"), () => {
      ...
      const buttonWidget = screen.getByTestId("baseButton-".concat(kind));
      expect(buttonWidget).toBeInTheDocument();
    });
    it("renders disabled ".concat(kind, " correctly"), () => {
      render( /*#__PURE__*/_jsx(BaseButton, {
        ...getProps({
          kind,
          disabled: true
  ...
  Object.keys(BaseButtonSize).forEach(key => {
    ...
    it("renders ".concat(size, " buttons correctly"), () => {
      render( /*#__PURE__*/_jsx(BaseButton, {
        ...getProps({
          size
        }),
      ...
      const expectedPadding = {
        [BaseButtonSize.XSMALL]: "".concat(spacing.twoXS, " ").concat(spacing.sm),
        [BaseButtonSize.SMALL]: "".concat(spacing.twoXS, " ").concat(spacing.md),
        [BaseButtonSize.LARGE]: "".concat(spacing.md, " ").concat(spacing.md),
        [BaseButtonSize.MEDIUM]: "".concat(spacing.xs, " ").concat(spacing.md)
      };
      const buttonWidget = screen.getByRole("button");
      expect(buttonWidget).toHaveStyle("padding: ".concat(expectedPadding[size]));
    });
  });
